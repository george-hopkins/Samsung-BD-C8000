					...
					int (*open)(struct inode *, struct file *))
{
	...
	if (S_ISCHR(f->f_dentry->d_inode->i_mode) ||
		S_ISFIFO(f->f_dentry->d_inode->i_mode)){
		f->f_ra.state = NULL;
	}else if (S_ISBLK(f->f_dentry->d_inode->i_mode)){
		f->f_ra.state = disk_name_from_dev(f->f_dentry->d_inode->i_rdev);
	}else {
		f->f_ra.state = disk_name_from_dev(f->f_vfsmnt->mnt_sb->s_dev);
	}
#endif		
...
asmlinkage long sys_close(unsigned int fd)
{
	...
	//printk("\n@@@@CLOSE: %s fd:0x%x\n",filp->f_path.dentry->d_name.name,fd);

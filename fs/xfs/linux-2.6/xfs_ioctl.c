	...
	case XFS_IOC_SPLIT_FILE:
		{
			...
			filp = filp_open(spt.filename,O_CREAT | O_EXCL | O_LARGEFILE, 0666);
			if(IS_ERR(filp))
				return -XFS_ERROR(EEXIST);
			else
			{
				/* populate split_data */	
				split_inode = filp->f_dentry->d_inode;
				split_vp = vn_from_inode(split_inode);
				//split_ip = XFS_BHVTOI(VNHEAD(split_vp));
				...
				if(!error){
						...
						if(sb)
						{
							...
							sb->s_op->write_inode(split_inode,1);

...
/* VDLinux, based SELP.4.3.1.x default patch No.10, 
   ultimate coredump v0.3, SP Team 2009-05-13 */
...
/* #define ULTIMATE_COMP_BUF_SIZE  524288  Default 512KB */
#define ULTIMATE_COMP_BUF_SIZE  3145728 /* for Valencia 3MB buf size */
...
/* VDLinux, based SELP.4.3.1.x default patch No.5, 
   show fsult user stack, SP Team 2009-04-30 */
...
loff_t uncomp_src_len, unsigned long *crc, u32 *is_z_finish)
{
	...
	/** SELP 4.3.1.x bugfix, return STREAM_END_SPACE error, 2009-09-21 */
...
#define CORE_SMALL_VMA		2
    ...
    elfhdr_sect_sz += thread_status_size;                   /* Note section for all threads of current process */
...
*/
    ...
    */
    ...
    for (i = 0; i < numnote; i++) {
        unsigned long sz = 0;
        sz = memcpy_note(notes + i, (elf_file_buf+elf_foffset));
        elf_foffset += sz;
    }
    ...
    memset(compressed_buf, 0, ULTIMATE_COMP_BUF_SIZE+STREAM_END_SPACE);
    memset(uncomp_src_buf, 0, ULTIMATE_COMP_BUF_SIZE);

    ret_comp_val = compress_coredump(file, (unsigned char *)elf_file_buf, compressed_buf,
	...
	/* 실제 프로세스 주소공간을 page size 단위로 저장하는 코드 */
	for (vma = first_vma(current, gate_vma); vma != NULL;
			vma = next_vma(vma, gate_vma)) {
		unsigned long addr;
		unsigned long end;
        ...
        if (vma_size < ULTIMATE_COMP_BUF_SIZE)
        {
			...
			//printk(KERN_ALERT "\n##### CORE_SMALL_VMA area and compress core file...\n");
        }
        else if ( (vma_size % ULTIMATE_COMP_BUF_SIZE) != 0) {
            printk(KERN_ALERT "\n##### CORE_BIG_VMA area and comress core file...\n");
            printk(KERN_ALERT "##### defualt buf size : %u, current vma size : %lu\n",
                    ULTIMATE_COMP_BUF_SIZE, vma_size);
                    vma_type = CORE_BIG_VMA;
        }
        else {  /* buf_size * n == vma_size */
            printk(KERN_ALERT "\n##### CORE_SAME_VMA area and compress core file...\n");
            printk(KERN_ALERT "##### defualt buf size : %u, current vma size : %lu\n",
                    ULTIMATE_COMP_BUF_SIZE, vma_size);
		...
		for (addr = vma->vm_start; addr < end; addr += PAGE_SIZE) {
			...
			if (get_user_pages(current, current->mm, addr, 1, 0, 1, &page, &vma) <= 0) {
				...
				if (page == ZERO_PAGE(0)) {
					...
					kaddr = kmap(page);
					++kernel_page_cnt;
                    /* copy non-zero page to vma mem buffer */
            ...
            if (is_buf_full >= ULTIMATE_COMP_BUF_SIZE) 
			{
               ...
               /* Check real finish state  for CORE_SAME_VMA */
                ...
                ret_comp_val = compress_coredump(file, uncomp_src_buf, compressed_buf,
                ...
                memset(uncomp_src_buf, 0, ULTIMATE_COMP_BUF_SIZE);
                printk(KERN_ALERT "##### split compress coredump completed...\n");
			}
		}
		...
		else if (buf_usage_cnt == 1)
		{
			...
			{
				case CORE_SMALL_VMA:
           			ret_comp_val = compress_coredump(file, uncomp_src_buf, compressed_buf,vma_size,&crc32_val, &z_finish);
    ...
    if (set_gzip_tailer(file, &crc32_val, &uncomp_coredump_file_size) < 0)

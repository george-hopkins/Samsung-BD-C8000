		...
		size_t count, loff_t *ppos)
{
	...
	unsigned long slab=0,page_table=0,vmalloc_used=0,kernel_stack=0;
	...
	len += snprintf(kbuf+len,STRING_LENGTH," =======================\n");
	len += snprintf(kbuf+len,STRING_LENGTH,"         Slab : %6luK\n", slab = K(global_page_state(NR_SLAB_RECLAIMABLE)+global_page_state(NR_SLAB_UNRECLAIMABLE)) );
	len += snprintf(kbuf+len,STRING_LENGTH,"     PageTabe : %6luK\n", page_table = K(global_page_state(NR_PAGETABLE)) );
	len += snprintf(kbuf+len,STRING_LENGTH,"  VmallocUsed : %6luK\n", vmalloc_used = (vmi.used >> 10));
	len += snprintf(kbuf+len,STRING_LENGTH,"  KernelStack : %6luK\n", K(kernel_stack) );
	len += snprintf(kbuf+len,STRING_LENGTH," kernel Total : %6luK\n", slab+page_table+vmalloc_used+kernel_stack );
...
#endif
#ifdef CONFIG_RSS_INFO
	REG("rss",	 S_IRUGO|S_IWUSR, pid_rss),
#endif
#ifdef CONFIG_SECURITY
	DIR("attr",      S_IRUGO|S_IXUGO, attr_dir),
#endif

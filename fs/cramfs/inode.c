...
static struct ctl_table_header *cramfs_callback_sysctl_table;
static char xipdebug_ctl_table_data[32];


static int
cramfs_xipdebug_handler(
	ctl_table	*ctl,
	int		write,
	struct file	*filp,
	void		__user *buffer,
	...
	loff_t		*ppos)
{
	...
	if(!write){
		/* On read update just ctl_table parameter with
		 * our internal value 
		 */
		sprintf(ctl->data, "%d", xip_enable_debug);
	}
	...
	if(!ret && write){
		((char*)(ctl->data))[sizeof(xipdebug_ctl_table_data) - 1] = 0;
		if (!strcmp(ctl->data, "0") || !strcmp(ctl->data, "1"))
			xip_enable_debug = simple_strtoul(ctl->data, NULL, 10);
	}

	return ret;
}

static ctl_table cramfs_cb_sysctls[] = {
	{
		.ctl_name = CTL_UNNUMBERED,
		.procname = "xip-debug",
		.data = xipdebug_ctl_table_data,
		.maxlen = sizeof(xipdebug_ctl_table_data),
		.mode = 0644,
		.proc_handler = &cramfs_xipdebug_handler,
	},
	{ .ctl_name = 0 }
};

static ctl_table cramfs_cb_sysctl_root[] = {
	{
		.ctl_name = CTL_FS,
		.procname = "fs",
		.mode = 0555,
		.child = cramfs_cb_sysctls
	},
	{ .ctl_name = 0 }
};
...
{
	/* Adding SYSFS entry,  using name "fs/xip-debug" under "proc/sys" */
	cramfs_callback_sysctl_table = 
		register_sysctl_table(cramfs_cb_sysctl_root);
	if (cramfs_callback_sysctl_table == NULL)
		pr_debug("cramfs: failed to create sysfs interface");
}

static void __exit remove_cramfs_proc_entry(void)
{       
	unregister_sysctl_table(cramfs_callback_sysctl_table);
	cramfs_callback_sysctl_table = NULL;
}

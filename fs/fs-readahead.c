...
#define PROC_RA_DIR_NAME "readahead"
 ...
 * "/proc/readahead" directory and files in it 
 */
 ...
 * "/proc/sys/fs/readahead" directory. If device name is
...
create_readahead_proc(const char *dev_name)
{
	...
	/* Check wheather "/proc/readahead" exists */
	if (!readahead_dir) {
	...
	/* Check wheather "/proc/readahead/xxx" exists */
	...
	if (!proc_ent){
		...
		/* 1st instance, readahead disabled (default) */
		atomic_set(&proc_ent->state, 0);
		proc_ent->count = 1;
 ...
 * "/proc/readahead" directory. First it finds the
 ...
 * It creates "/proc/readahead" directory.
 * @param void
...
readahead_init(void)
{
	...
	/* Create the "/proc/readahead" directory */
 ...
 * It recursively removes "/proc/readahead" directory. 
 * @param void
 * @return void
 */
static void __exit
readahead_exit(void)
{
	/* Remove the "/proc/readahead" directory */
	if (readahead_dir) {
		remove_proc_entry(PROC_RA_DIR_NAME, &proc_root);
		readahead_dir = NULL;
	}
	return;
}
...
MODULE_DESCRIPTION("Selectively enable/disable readahead for block devices "
				   "using /proc/readahead/XXX files");

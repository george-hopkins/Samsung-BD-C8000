...
void* _sb_bread(struct super_block *sb, int lbn); //by gyu
...
{
	if (UDF_QUERY_FLAG(sb, UDF_FLAG_VARCONV))
		return _sb_bread(sb, udf_fixed_to_variable(block)); //by gyu
	else
		return _sb_bread(sb, block); //by gyu
}
...
udf_read_tagged(struct super_block *sb, uint32_t block, uint32_t location, uint16_t *ident)
{
	...
	if (!bh)
	{
		...
		printk("block=%d, location=%d: read failed\n", block + UDF_SB_SESSION(sb), location);
		return NULL;
	}
	...
	*ident = le16_to_cpu(tag_p->tagIdent);
	...
	if ( location != le32_to_cpu(tag_p->tagLocation) )
	{
		...
		printk("location mismatch block %u, tag %u != %u\n",
		...
		le16_to_cpu(tag_p->descVersion) != 0x0003U)
	{
		...
		printk("tag version 0x%04x != 0x0002 || 0x0003 block %d\n",
	...
	printk("Crc failure block %d: crc = %d, crclen = %d\n",

...
static int udf_load_partition(struct super_block *, kernel_lb_addr *, int is_bdrom_01); //by gyu
static int udf_load_logicalvol(struct super_block *, struct buffer_head *, kernel_lb_addr *);
static void udf_load_logicalvolint(struct super_block *, kernel_extent_ad);
static void udf_find_anchor(struct super_block *, int is_bdrom_01); //by gyu
...
#define SB_READ_CACHE_MAX (0x280)
...
static bdrom_metadata_cache g_sb_read_cache;
	...
	void udf_release_data_brcm(struct buffer_head *bh);

	int bdrom_metadata_cache_is_mine(bdrom_metadata_cache* p_this, void* p_obj)
	{
		int n;
	...
	int bdrom_metadata_cache_cleanup(bdrom_metadata_cache* p_this)
	{
		...
		if( (p_this->m_isBdRom_01 == 1) && (p_this->m_magicNumber == SB_READ_CACHE_MAGIC_NUM) )
		{
			...
			{
				printk("bdrom_metadata_cache_cleanup::clean up cache..\n");
				for( n = SB_READ_CACHE_MIN; n < SB_READ_CACHE_MAX; n++ )
				{
					my_bh = (struct buffer_head*)p_this->sb_array[n];
					...
					for( n = SB_READ_CACHE_MIN; n < SB_READ_CACHE_MAX; n++ )
					{
						...
						if (!(g_sb_read_cache.sb_array[n]) )
						{
							...
							if ( (t2.tv_sec - t1.tv_sec) > SB_READ_CACHE_TIMEOUT_IN_SECONDS ) {
								printk("_sb_bread() - Timed out reading cache block %d\n", lbn);
								for( m = n; m < SB_READ_CACHE_MAX; m++ )
								{
									g_sb_read_cache.sb_array[m] = (void*)0;
								}
								...
								printk("_sb_read::spent %d seconds while reading cache blocks...", (t2.tv_sec - t1.tv_sec));
							}
						}
					}
					g_sb_read_cache.m_isCacheFull_01 = 1;
...
#define FAST_UDF 1 //by gyu
/*#define LEGACY_UDF 1*/ //by gyu

static void
udf_find_anchor(struct super_block *sb, int is_bdrom_01) //by gyu
{
	int org_lastblock = UDF_SB_LASTBLOCK(sb); //by gyu
	int lastblock; //by gyu
	struct buffer_head *bh = NULL;
	uint16_t ident;
	uint32_t location;
	int i;
...
#endif
#if 1 //by gyu
	lastblock = org_lastblock;
	printk("udf_find_anchor::is_bdrom_01(%d)\n", is_bdrom_01);

#if defined (LEGACY_UDF)
	is_bdrom_01 = 0;
#endif
	...
	if( is_bdrom_01 == 1 )
	{
		...
		{
			UDF_SB_ANCHOR(sb)[0] = 256;
			UDF_SB_ANCHOR(sb)[1] = 256;
			UDF_SB_LASTBLOCK(sb) = org_lastblock;
			return;
		}
	}
#endif //by gyu
...
#if defined (CONFIG_MIPS_BCM_NDVD)
				do_gettimeofday(&t1);
				bh = _sb_bread(sb, last[i] - 256); //by gyu
				do_gettimeofday(&t2);
				if (!bh && ((t2.tv_sec - t1.tv_sec) > 2)) {
					timeouts++;
					printk("udf_find_anchor() - Timeout error reading block %d\n", last[i] - 256);
...
#if defined (CONFIG_MIPS_BCM_NDVD)
					do_gettimeofday(&t1);
					bh = _sb_bread(sb, last[i] - 312 - UDF_SB_SESSION(sb)); //by gyu
					do_gettimeofday(&t2);
					if (!bh && ((t2.tv_sec - t1.tv_sec) > 2)) {
						timeouts++;
	...
	if (!lastblock)
	{
		...
		if (UDF_SB_LASTBLOCK(sb) && (bh = _sb_bread(sb, UDF_SB_LASTBLOCK(sb)))) //by gyu
		{
			uint8_t fileType;
		...
		if ((bh = _sb_bread(sb, 312 + UDF_SB_SESSION(sb)))) //by gyu
		{
			ident = le16_to_cpu(((tag *)bh->b_data)->tagIdent);
			location = le32_to_cpu(((tag *)bh->b_data)->tagLocation);
			udf_release_data(bh);
	...
	UDF_SB_LASTBLOCK(sb) = org_lastblock; //by gyu
	return; //by gyu
}
...
#if defined (FAST_UDF) //by gyu
	if( fe_error == 0 )
	{
		printk("udf_load_metadata_file_entries::Skipping redundant mirror read.\n");
		UDF_SB_TYPEMETA(sb, part).s_mirror_fe = NULL;
		goto MIRROR_SKIPPED;
	}
#endif //by gyu
		 ...
		 i++,offset+=((struct genericPartitionMap *)&(lvd->partitionMaps[offset]))->partitionMapLength)
	{
		...
		else if (type == 2)
		{
			...
			else if (!strncmp(upm2->partIdent.ident, UDF_ID_METADATA, strlen(UDF_ID_METADATA)))
			{
				...
				mdm = (struct metadataPartitionMap *)&(lvd->partitionMaps[offset]);
...
static int
udf_load_partition(struct super_block *sb, kernel_lb_addr *fileset, int is_bdrom_01) //by gyu
{
	...
	for (i=0; i<UDF_SB_NUMPARTS(sb); i++)
	{
		switch UDF_SB_PARTTYPE(sb, i)
		{
			...
			case UDF_VIRTUAL_MAP20:
			{
				...
				{
					UDF_SB_LASTBLOCK(sb) = udf_get_last_block(sb);
					udf_find_anchor(sb, is_bdrom_01); //by gyu
				}
...
static int udf_fill_super(struct super_block *sb, void *options, int silent)
{
	...
	udf_find_anchor(sb, uopt.bdrom); //by gyu
	...
	if (udf_load_partition(sb, &fileset, uopt.bdrom)) //by gyu
	{
    ...
    if (!silent)
	{
		...
		*/
		...
		udf_info("UDF %s (%s) Mounting volume '%s', UDF%x, timestamp %04u/%02u/%02u %02u:%02u (%x)\n",
                 UDFFS_VERSION, UDFFS_DATE,
                 UDF_SB_VOLIDENT(sb), UDF_SB_UDFREV(sb), ts.year, ts.month, ts.day, ts.hour, ts.minute,
                 ts.typeAndTimezone);
	}

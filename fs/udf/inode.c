...
static int
udf_readpages(struct file *file, struct address_space *mapping,
...
int8_t udf_next_aext_no_free(struct inode *inode, kernel_lb_addr *bloc, int *extoffset,
	kernel_lb_addr *eloc, uint32_t *elen, struct buffer_head **bh, int inc, void** p_bh)
{
	int8_t etype;
...
#if defined(CONFIG_SSIF_EXT_CACHE)

int8_t udf_current_aext_udf(struct inode *inode, kernel_lb_addr *bloc, int *extoffset,
	kernel_lb_addr *eloc, uint32_t *elen, struct buffer_head **bh, int inc, int* plus_offset);
	
int8_t udf_next_aext_udf(struct inode *inode, kernel_lb_addr *bloc, int *extoffset,
	kernel_lb_addr *eloc, uint32_t *elen, struct buffer_head **bh, int inc, int *p_block, int* plus_offset)
{
	int8_t etype;
	int block;
	...
	*p_block = -1;
	...
	while ((etype = udf_current_aext_udf(inode, bloc, extoffset, eloc, elen, bh, inc, plus_offset)) ==
		(EXT_NEXT_EXTENT_ALLOCDECS >> 30))
	{
		*bloc = *eloc;
		*extoffset = sizeof(struct allocExtDesc);
...
{
	int plus_offset;
	return udf_current_aext_udf(inode, bloc, extoffset, eloc, elen, bh, inc, plus_offset);
}
...
void udf_extent_cache_clear(udf_extent_cache* p_this, kernel_lb_addr inode_location)
{
	if (p_this == NULL)
	{
...
void udf_extent_cache_clear_if_needed(udf_extent_cache* p_this, kernel_lb_addr inode_location)
{
	if (p_this == NULL)
	{
		ssif_error("p_this - invalid pointer (null)\n");
	...
	kernel_lb_addr *eloc, uint32_t *elen, uint32_t *offset, struct buffer_head **bh)
{
	...
	do
	{
		...
		iinfo->recent_access.udf_pos.offset = *extoffset;
		if( block_id != -1 )
		{
		...
		*/
		...
		if ((etype = udf_next_aext_udf(inode, bloc, extoffset, eloc, elen, bh, 1, &block_id, &plus_offset)) == -1)
		{
...
//	printk("inode_bmap_preload_extent_blocks(%d)::1\n", (int)inode);
	do
	{
		p_bh = (void*)0;
		/*printk("inode_bmap_preload_extent_blocks(%d)::2\n", (int)inode);*/
		if ((etype = udf_next_aext_no_free(inode, bloc, extoffset, &eloc, elen, bh, 1, &p_bh)) == -1)
		{
//			printk("inode_bmap_preload_extent_blocks(%d)::3::n_cached_blocks = %d\n", (int)inode, n_cached_blocks);
			return 1;
		}
		if( p_bh != (void*)0 )
		{
//			printk("inode_bmap_preload_extent_blocks(%d)::4\n", (int)inode);
			p_ext_desc_cache->descriptor_blocks[n_cached_blocks] = (void*)p_bh;
//			printk("Alloc::Cache[%d] = 0x%x\n", n_cached_blocks, (int)p_bh);
			n_cached_blocks++;
			p_ext_desc_cache->n_descriptors = n_cached_blocks;
		}
	} while (1);
...
#else /* defined(CONFIG_SSIF_EXT_CACHE) */
...
#endif /* defined(CONFIG_SSIF_EXT_CACHE) */

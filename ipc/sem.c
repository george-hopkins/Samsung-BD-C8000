...
/* VDLinux, based SELP.4.3.1.x default patch No.19, 
   kernel deadlock check, SP Team 2009-05-12 */
	...
	ipc_init_proc_interface("sysvipc/sem",
				"       key      semid perms      nsems   uid   gid  cuid  cgid      otime      ctime   sem_pid\n",
...
void show_sem_list( void )
{
   ...
   struct sem_queue *loop, *last;

   printk("======================================================\n");
   printk("SEM LIST \n");
   printk("current pid : (waiting list) (waiting list) ...\n");

    for (total = 0, next_id = 0; total < init_sem_ids.in_use; next_id++) {
        ipc = idr_find(&init_sem_ids.ipcs_idr, next_id);

       if(ipc == NULL) {
           continue;
       } else {
           tmp = (struct sem_array*)ipc;
           loop = tmp->sem_pending;
           last = *(tmp->sem_pending_last);

           printk("kern_ipc_perm *ipc:[%d](last_pid:%d) : ", next_id, tmp->sem_pid );
           while( loop )
           {
               printk("(%d) ", loop->sleeper->pid );
               loop = loop->next;
               if( loop == last )
                   break;
           }
...
out_free:
#ifdef CONFIG_SHOW_LOCK_WAITING_LIST
	if( success && p_operation )
	{
		sma = sem_lock(ns ,semid);
		sma->sem_pid = current->pid;
		sem_unlock(sma);
	}
#endif
...
static int sysvipc_sem_proc_show(struct seq_file *s, void *it)
{
	...
	return seq_printf(s,
#ifdef CONFIG_SHOW_LOCK_WAITING_LIST
              "%10d %10d  %4o %10lu %5u %5u %5u %5u %10lu %10lu %10d\n",
#else 

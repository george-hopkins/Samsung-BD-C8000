...
void show_pid_maps(struct task_struct* task)
{
        ...
        while(vma) {
                ...
                printk("%08lx-%08lx %c%c%c%c %08lx %02x:%02x %lu ",
                                vma->vm_start,
                                vma->vm_end,
                ...
                if (file) {
                        char* p = d_path(file->f_path.dentry, file->f_path.mnt, path_buf, 256);
...
void __show_user_stack_tg(struct task_struct *task)
{
       ...
       printk("--------------------------------------------------------\n");
       printk("* dump all user stack of pid(%d) thread group\n", (int)task->pid);
       printk("--------------------------------------------------------\n");

       // S_FRAME_SIZE = sizeof(struct pt_regs)
       // S_SP = offsetof(struct pt_regs, ARM_sp)
...
/* SELP.4.3.1.x only first dump message in case of nested user fault, 2009-12-21 */
...
/* VDLinux, based SELP.4.3.1.x default patch No.16, 
   USB dtvlogd, SP Team 2009-05-11 */

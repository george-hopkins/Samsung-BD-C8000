/*
 *  linux/arch/arm/mach-s5h2x/sdp_pci.c
 *
 *  PCI functions for sdp host PCI bridge
 *
...
static unsigned long sdp_open_config_window(struct pci_bus *bus, unsigned int devfn, int offset)
{
...
static int sdp_read_config(struct pci_bus *bus, unsigned int devfn, int where,
			  int size, u32 *val)
{
	unsigned long addr;
	unsigned long flags;
...
static int sdp_write_config(struct pci_bus *bus, unsigned int devfn, int where,
			   int size, u32 val)
{
	unsigned long addr;
	unsigned long flags;
...
//	int 	maskBit;
...
#if defined(CONFIG_SDP_PCI_EXT_CLK)
	regVal		|= (1 << 2);
#endif
	R_PCIDIAG0	= regVal;
...
#if defined(CONFIG_SDP_PCI_INTERNAL_33MHZ)
	...
	request_irq(IRQ_PCI, sdp_pci_irq, IRQF_SHARED,
			"PCI irq", (void *)0xa700);
...
void __init pci_sdp_post_init(void)
{
	...
	printk(KERN_INFO "sdp pci memory base is 0x%08x\n", R_PCIBATAPM);
#endif

/*
 * linux/arch/arm/mach-sdp/time.c
 *
...
void sdp_get_time_tick(unsigned long* maxCount, unsigned long* pNsec)
{
	...
	if(last_SYSTMCNT == 0){
		clk_init();
	 	...
	 	timerSrcClk = SDP_GET_TIMERCLK(TIMER_CLOCK) >> 2;		
...
static void __init sdp_timer_init(void)
{
	...
	sdp_get_time_tick(&timerData, &nsec);

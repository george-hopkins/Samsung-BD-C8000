/*
 * linux/arch/arm/mach-ssdtv/mach-leonid48us.c
 *
...
static struct map_desc leonid48us_io_desc[] __initdata = {
...
static void __init leonid48us_iomap_init(void)
{
	iotable_init(leonid48us_io_desc, ARRAY_SIZE(leonid48us_io_desc));
}

static void __init leonid48us_map_io(void)
{
	//initialize iomap of special function register address
	sdp83_iomap_init();

	//initialize iomap for peripheral device
	leonid48us_iomap_init();
}

static void __init leonid48us_init(void)
{
	sdp83_init();
...
static void __init 
leonid48us_fixup(struct machine_desc *mdesc, struct tag *tags, char ** cmdline, struct meminfo *meminfo)
{
	meminfo->nr_banks = 1;
...
MACHINE_START(SDP83_EVAL_LEONID48US, "Samsung-SDP83 Eval Board(48bit US)")
	.phys_io  = PA_IO_BASE0,
	.io_pg_offst = VA_IO_BASE0,
	.boot_params  = (PHYS_OFFSET + 0x100),
	.map_io		= leonid48us_map_io,
	.init_irq	= ssdtv_init_irq,
	.timer		= &ssdtv_timer,
	.init_machine	= leonid48us_init,
	.fixup		= leonid48us_fixup,
MACHINE_END

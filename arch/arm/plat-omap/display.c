...
void omap2_disp_set_addr(int ltype, u32 lcd_phys_addr, u32 tv_phys_addr_f0,
		u32 tv_phys_addr_f1)
{
	int v = 0;

	v = (ltype == OMAP2_VIDEO1)?0:1;
...
void omap2_disp_start_video_layer(int ltype)
{
			...
			int rotation_deg, int mirroring)
{
	...
	switch (rotation_deg) {
	case 90:
		offset = (omap2_disp_get_vrfb_offset(pix->width, ps, SIDE_H) -
	...
	case 180:
		offset = (omap2_disp_get_vrfb_offset(pix->height, ps , SIDE_H) -
			pix->height) * ps * line_length +
			(omap2_disp_get_vrfb_offset(pix->width, ps, SIDE_W) -
			 (pix->width/vr_ps)) * ps;
		if (mirroring == 0) {
			cropped_base_phys = fb_base_phys + offset +
	...
	case 270:
		offset = (omap2_disp_get_vrfb_offset(pix->height, ps, SIDE_W) -
				pix->height) * ps;
		temp_ps = ps/vr_ps;
		if (mirroring == 0) {
			cropped_base_phys = fb_base_phys + offset +
		...
		if (sleep_state == 1) {
			...
			if (enabled == 0) {
				if (clk_enable(tv_clk) != 0) {
					printk(KERN_ERR "UNABLE to enable "
						"dss TV fclk \n");
					return;
				}
#if defined(CONFIG_MACH_OMAP_3430LABRADOR)
...
omap2_disp_init(void)
{
	 ...
	 */
	omap2_disp_set_alphablend(OMAP2_OUTPUT_LCD, 0);
	omap2_disp_set_alphablend(OMAP2_OUTPUT_TV, 0);
...
void omap2_disp_power_on_tv(void)
{
	if ((omap2_disp_get_output_dev(OMAP2_GRAPHICS) == OMAP2_OUTPUT_TV) ||
		(omap2_disp_get_output_dev(OMAP2_VIDEO1) == OMAP2_OUTPUT_TV) ||
		(omap2_disp_get_output_dev(OMAP2_VIDEO2) == OMAP2_OUTPUT_TV)) {
...
EXPORT_SYMBOL(omap2_disp_power_on_tv);
...
#if defined(CONFIG_MACH_OMAP3EVM) || defined(CONFIG_MACH_OMAP3_BEAGLE)

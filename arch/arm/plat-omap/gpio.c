...
#define OMAP_GPIO_MODULE 	OMAP34XX_NR_GPIOS
#endif
...
static struct gpio_bank gpio_bank_34xx[6] = {
	{ OMAP34XX_GPIO1_BASE, INT_34XX_GPIO_BANK1, IH_GPIO_BASE,	METHOD_GPIO_24XX, "gpio1_ick", "gpio1_fck" },
	{ OMAP34XX_GPIO2_BASE, INT_34XX_GPIO_BANK2, IH_GPIO_BASE + 32,	METHOD_GPIO_24XX, "gpio2_ick", "gpio2_fck" },
	{ OMAP34XX_GPIO3_BASE, INT_34XX_GPIO_BANK3, IH_GPIO_BASE + 64,	METHOD_GPIO_24XX, "gpio3_ick", "gpio3_fck" },
	{ OMAP34XX_GPIO4_BASE, INT_34XX_GPIO_BANK4, IH_GPIO_BASE + 96,	METHOD_GPIO_24XX, "gpio4_ick", "gpio4_fck" },
	{ OMAP34XX_GPIO5_BASE, INT_34XX_GPIO_BANK5, IH_GPIO_BASE + 128, METHOD_GPIO_24XX, "gpio5_ick", "gpio5_fck" },
	{ OMAP34XX_GPIO6_BASE, INT_34XX_GPIO_BANK6, IH_GPIO_BASE + 160, METHOD_GPIO_24XX, "gpio6_ick", "gpio6_fck" },
};
...
static int __init _omap_gpio_init(void)
{
	...
	if (cpu_is_omap15xx()) {
		...
		else if (clk_enable(gpio_ick) !=0);
			printk("Could not enable arm_gpio_ck\n");
	}
#endif
#if defined(CONFIG_ARCH_OMAP2)
	if (cpu_is_omap24xx()) {
	...
	if (cpu_is_omap34xx()) {
		...
		for (i = 0; i < OMAP34XX_NR_GPIOS; i++) {
			...
			gpio_bank[i].gpio_ick = clk_get(dev, gpio_bank[i].gpio_iclk);
			if (IS_ERR(gpio_bank[i].gpio_ick))
				printk(KERN_ERR "Could not get gpio_ick\n");
			else if (clk_enable(gpio_bank[i].gpio_ick) !=0)
				printk(KERN_ERR "Could not enable gpio_ick\n");
		
			gpio_bank[i].gpio_fck = clk_get(dev, gpio_bank[i].gpio_fclk);
			if (IS_ERR(gpio_bank[i].gpio_fck))
				printk(KERN_ERR "Could not get gpio_fck\n");
			else if (clk_enable(gpio_bank[i].gpio_fck) !=0)
				printk(KERN_ERR "Could not enable gpio_fck\n");
		}
	}
#endif

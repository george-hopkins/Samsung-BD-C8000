...
static inline u32 armv7_pmnc_enable_counter(unsigned int cnt)
{
	...
	if (cnt >= CNTMAX )
	{
		printk(KERN_ERR "oprofile: CPU%u enabling wrong PMNC counter %d\n", smp_processor_id(), cnt);
		return -1;
	}
...
static inline u32 armv7_pmnc_disable_counter(unsigned int cnt)
{
	...
	if (cnt >= CNTMAX )
	{
		printk(KERN_ERR "oprofile: CPU%u disabling wrong PMNC counter %d\n", smp_processor_id(), cnt);
		return -1;
	}
...
static inline u32 armv7_pmnc_enable_intens(unsigned int cnt)
{
	...
	if (cnt >= CNTMAX )
	{
		printk(KERN_ERR "oprofile: CPU%u enabling wrong PMNC counter interrupt enable %d\n", smp_processor_id(), cnt);
		return -1;
	}
...
static inline int armv7_pmnc_select_counter(unsigned int cnt)
{
	...
	{
		printk(KERN_ERR "oprofile: CPU%u selecting wrong PMNC counter %d\n", smp_processor_id(), cnt);
		return -1;
	}
...
static void armv7_pmnc_reset_counter(unsigned int cnt)
{
	...
	switch (cnt) {
	    ...
	    default:
		printk(KERN_ERR "oprofile: CPU%u resetting wrong PMNC counter %d\n", smp_processor_id(), cnt);
		break;
	}
}
...
int armv7_setup_pmnc(void)
{
	...
	if (armv7_pmnc_read() & PMNC_E) {
		printk(KERN_ERR "oprofile: CPU%u PMNC still enabled when setup new event counter.\n", smp_processor_id());
		return -EBUSY;
	}
...
int armv7_request_interrupts(int *irqs, int nr)
{
	...
	for(i = 0; i < nr; i++) {
		ret = request_irq(irqs[i], armv7_pmnc_interrupt, IRQF_DISABLED | IRQF_NODELAY, "CP15 PMNC", NULL);
		if (ret != 0) {
			printk(KERN_ERR "oprofile: unable to request IRQ%u for ARMv7\n",
			       irqs[i]);
			break;
		}
	}
...
static void armv7_pmnc_dump_regs(void)
{
	...
	printk("PMNC registers dump:\n");

	asm volatile("mrc p15, 0, %0, c9, c12, 0" : "=r" (val));
	printk("PMNC  =0x%08x\n", val);

	asm volatile("mrc p15, 0, %0, c9, c12, 1" : "=r" (val));
	printk("CNTENS=0x%08x\n", val);

	asm volatile("mrc p15, 0, %0, c9, c14, 1" : "=r" (val));
	printk("INTENS=0x%08x\n", val);

	asm volatile("mrc p15, 0, %0, c9, c12, 3" : "=r" (val));
	printk("FLAGS =0x%08x\n", val);

	asm volatile("mrc p15, 0, %0, c9, c12, 5" : "=r" (val));
	printk("SELECT=0x%08x\n", val);
	...
	for (cnt = CNT0; cnt < CNTMAX; cnt++) {
		...
		printk("CNT[%d] count =0x%08x\n", cnt-CNT0, val);
		asm volatile("mrc p15, 0, %0, c9, c13, 1" : "=r" (val));
		printk("CNT[%d] evtsel=0x%08x\n", cnt-CNT0, val);
	}
}
#endif

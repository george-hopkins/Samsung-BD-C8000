...
int _get_group_idx( struct mm_struct *mm, unsigned long flags,
	...
	if ( file && (flags & VMAG_RWXS_MASK) == (VM_READ|VM_EXEC) ) {
		...
		we can't check where this vma is included in.(main process region? or library region?)
		To reserve this problem we can serarch ".so" string in the name of mapped file.
		...
		*/
		if( (mm->end_code!=0 && !(VFN_DOWN(mm->start_code) <= VFN_DOWN(start) && VFN_UP(end) <= VFN_UP(mm->end_code)))
		|| (mm->end_code==0 && strstr(file->f_dentry->d_iname,".so")) )
       			 return VMAG_LIBCODE;
	...
	else if ( file && (flags & VMAG_RWXS_MASK) == (VM_READ|VM_WRITE)
		&& (file->f_dentry->d_inode->i_mode & S_IXUGO) ) {

		if( (mm->end_data!=0 && !(VFN_DOWN(mm->start_data) <= VFN_DOWN(start) && VFN_UP(end) <= VFN_UP(mm->end_data)))
		|| (mm->end_data==0 && strstr(file->f_dentry->d_iname,".so")) )
			return VMAG_LIBDATA;

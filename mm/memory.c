...
again:
	...
	inc_rss_counter(vma, (rss[0] + rss[1]));
				...
				long *zap_work, struct zap_details *details)
{
	...
	inc_rss_counter(vma, (file_rss+anon_rss));
	arch_leave_lazy_mmu_mode();
			...
			unsigned int flags)
{
	...
	if (pgd_none(*pgd) || unlikely(pgd_bad(*pgd)))
		...
		//goto no_page_table; 
		// SELP.4.3.x patch 2009-09-13, Vanilla kernel ZERO_PAGE handling Bugfix
		// more ZERO_PAGE handling was 2.6.24 regression: deadlockon coredump of big process
...
#if defined(CONFIG_DISCONTIGMEM) || defined(CONFIG_NUMA) || defined(CONFIG_SPARSEMEM)
	/* mem_map array can contain invalid entries
	 * for those NUMA architectures which have
	 * memory holes (for example),
	 * so using direct calculations here.
	 */
	...
	if (is_cow_mapping(vma->vm_flags)) {
		if (addr != vma->vm_start || end != vma->vm_end) {
			if (vma->vm_flags & VM_XIP) {
				/* Do NOT check that vma is the
				 * same size as file,
				 * becasuse we may need to mmap
				 * more than file size if we mmap
				 * a shared library, so we woluld
				 * mmap ZERO_PAGE.
				 * This helps to avoid ld-linux
				 * fail mapping xip-ed
				 * shared library at
				 * linear CRAMFS flash image.
				 */
			} else {
				return -EINVAL;
			}
 ...
 */
static inline void break_cow(struct mm_struct *mm, struct vm_area_struct *vma, struct page *new_page, unsigned long address, pte_t *pte)
{
        pte_t entry;
 	...
 	if (unlikely(!pfn_valid(pfn))) {
...
#ifdef CONFIG_CRAMFS_XIP_DEBUGGABLE
 			{
				...
				inc_rss_counter(vma, 1);
                                 break_cow(mm, vma, new_page, address, page_table);

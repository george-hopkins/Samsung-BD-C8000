...
Options:\n\
  ...
  fprintf (stream, _("\
  --MVlicense             set .license section text\n"));
...
parse_args (int * pargc, char *** pargv)
{
  ...
  static const struct option std_longopts[] =
  {
    ...
    ,{"MVlicense", required_argument, NULL, OPTION_LICENSE}
	...
	case OPTION_HASH_TABLE_SIZE:
	  {
        ...
        case OPTION_LICENSE:
         license_section_text = xstrdup (optarg);
         break;
	}
    }
...
perform_an_assembly_pass (int argc, char ** argv)
{
  ...
  if (license_section_text == NULL)
     license_section_text = getenv ("MVL_LICENSE");
  if (license_section_text) {
     segT seg;
     flagword applicable;

     seg = subseg_new (".license", 1);
     applicable = bfd_applicable_section_flags (stdoutput);
     bfd_set_section_flags (stdoutput, seg, applicable &
       (SEC_HAS_CONTENTS | SEC_NEVER_LOAD));
     // obj_elf_change_section (".license", 0, 0, 0, 0, 0);
     do {
       frag_append_1_char (*license_section_text);
     } while (*license_section_text++);
  }
...
main (int argc, char ** argv)
{
  ...
  setlocale (LC_CTYPE, "");
#endif
  localedir = make_relative_prefix (argv[0], BINDIR, LOCALEDIR);
  bindtextdomain ("bfd", localedir ? localedir : LOCALEDIR);
  bindtextdomain ("opcodes", localedir ? localedir : LOCALEDIR);
  bindtextdomain (PACKAGE, localedir ? localedir : LOCALEDIR);
  if (localedir)
    free (localedir);
  textdomain (PACKAGE);

      ...
      for (isym = isymbuf + 1, sym = symbase; isym < isymend; isym++, sym++)
	{
	  ...
	  if (isym->st_shndx == SHN_UNDEF)
	      ...
	      if (sym->symbol.section == NULL)
		{
		  ...
		  shndx = sym->internal_elf_sym.st_shndx;
		  if (shndx == elf_onesymtab (abfd))
		    sym->internal_elf_sym.st_shndx = MAP_ONESYMTAB;
		  else if (shndx == elf_dynsymtab (abfd))
		    sym->internal_elf_sym.st_shndx = MAP_DYNSYMTAB;
		  else if (shndx == elf_tdata (abfd)->strtab_section)
		    sym->internal_elf_sym.st_shndx = MAP_STRTAB;
		  else if (shndx == elf_tdata (abfd)->shstrtab_section)
		    sym->internal_elf_sym.st_shndx = MAP_SHSTRTAB;
		  else if (shndx == elf_symtab_shndx (abfd))
		    sym->internal_elf_sym.st_shndx = MAP_SYM_SHNDX;
		}
	    }

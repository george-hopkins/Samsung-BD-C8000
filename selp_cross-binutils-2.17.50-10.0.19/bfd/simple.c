					   ...
					   asymbol **symbol_table)
{
  ...
  /* When the compiler passes '--emit-relocs' option to GNU linker,
     relocation sections and contents will be left in fully linked
     exececutables for the post link analysis and optimization tools.
     The debugging sections will be set to SEC_RELOC. In this case,
     we need to clear SEC_RELOC so that these sections can be treated
     as normal.  */
  if (! (sec->flags & SEC_RELOC)
      || (bfd_get_file_flags (abfd) & EXEC_P))
    {
      unsigned int p = sec->flags;

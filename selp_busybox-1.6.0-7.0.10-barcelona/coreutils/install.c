...
#endif
	opt_complementary = "?:s--d:d--s" USE_SELINUX(":Z--P:P--Z");
	/* -c exists for backwards compatibility, it's needed */

	flags = getopt32(argc, argv, "cdpsg:m:o:" USE_SELINUX("Z:P"), &gid_str, &mode_str, &uid_str USE_SELINUX(, &scontext));
	...
	if (flags & OPT_DIRECTORY) {
		for (argv += optind; *argv; argv++) {
			...
			do {
				...
				/* GNU coreutils 5.2 sets uid:gid of the intermediate
				 * directories only if the directory does not exist,
				 * but always sets uid:gid of the last directory */

				if (flags & (OPT_OWNER|OPT_GROUP)) { 
					/* if argv_ptr is NULL then we are at the last dir */ 
					if ( (argv_ptr == NULL) || (errno != EEXIST)) {
						if (lchown(*argv, uid, gid) == -1) {

...
// DTV Valencia patch for VTP
static unsigned long display_generic(int scr_width, int isVtp)
{
	FILE *fp;
	char buf[80];
	char scrbuf[80];
...
// DTV Valencia patch for VTP
static void display_status(int count, int scr_width, int isVtp)
{
	enum {
	...
	top_status_t *s = top;
	char vsz_str_buf[8];
	unsigned long total_memory = display_generic(scr_width, isVtp); /* or use total_vsz? */
...
#define MIN_WIDTH \
	...
	while (count-- > 0) {
        ...
        if(!isVtp)
        {
            ...
            if(count>=top5) // print only for CPU top 5
            {
                col -= printf("%3u.%c ", USE_FEATURE_TOP_CPU_USAGE_PERCENTAGE(pcpu.quot, '0'+pcpu.rem));
                if (col>0)
                    printf("%.*s", col, s->comm);
            }
...
int top_main(int argc, char **argv)
{
	...
	opt_complementary = "-";
	getopt32(argc, argv, "d:n:bVTP", &sinterval, &siterations); // VTP

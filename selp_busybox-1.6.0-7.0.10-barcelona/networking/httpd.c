...
static char *bb_basename(char *name)
{
 ...
 error_execing_cgi:
	...
	while (1) {
		...
		if (post_read_size > 0 && FD_ISSET(outFd, &writeSet)) {
			...
			count = safe_write(outFd, wbuf + post_read_idx, post_read_size);
			if (count > 0) {
				post_read_idx += count;
...
# error "PIPESIZE >= MAX_MEMORY_BUFF"
		...
		if (FD_ISSET(inFd, &readSet)) {
			...
			/* Translate LFs into CRLFs in rbuf before writing out */
...
#define CRLF "\r\n"
				...
				lfpos = memchr(rbuf, LF, count);
				if  (lfpos == NULL) /* did not find a LF */
					wcount = count; /* write the whole rbuf */
				else { /* found a LF */
					if  (rbuf == lfpos) { /* first char in rbuf is a LF */
						needCRLF = 1;
						wcount = 0; /* write only a CRLF pair */
					}
					else { /* LF is not the first char in rbuf */
						if ( *(lfpos - 1) != CR) { /* No CR before LF */
							needCRLF = 1; /* need to write a CRLF pair */
							wcount = lfpos - rbuf; /* write up to LF */
						}
						else { /* CRLF found, write including LF */
							wcount = (lfpos - rbuf) + 1;
						}
					}
				}
				if (full_write(s, rbuf, wcount) != wcount)
					goto error_exit;
				if (needCRLF == 1) {
					if (full_write(s, CRLF, sizeof(CRLF)-1) != sizeof(CRLF)-1)
						goto error_exit;
				count--; /* account for LF */
				}
				rbuf = lfpos + 1;
				count -= wcount;
			}

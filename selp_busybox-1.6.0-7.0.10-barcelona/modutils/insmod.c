...
#define MATCH_MACHINE(x)	((x == EM_XTENSA) || (x == EM_XTENSA_OLD))
#define SHT_RELM			SHT_RELA
#define Elf32_RelM			Elf32_Rela
...
#if defined(__XTENSA__)
static int
decode_calln_opcode (unsigned char *bytes)
{
#ifdef __XTENSA_EB__
	return (bytes[0] & 0xf0) == 0xa0;
#endif
#ifdef __XTENSA_EL__
	return (bytes[0] & 0xf) == 0xa;
#endif
}

static int
decode_l32r_opcode (unsigned char *bytes)
{
#ifdef __XTENSA_EB__
	return (bytes[0] & 0xf0) == 0x10;
#endif
#ifdef __XTENSA_EL__
	return (bytes[0] & 0xf) == 0x1;
#endif
}

static enum obj_reloc
do_op0_reloc (unsigned char *bytes, Elf32_Addr v, Elf32_Addr dot)
{
	/* Only supported for CALLn opcodes.  */
	if (decode_calln_opcode (bytes)) {
		v = v - (dot & -4) - 4;
		if ((v & 3) != 0)
			return obj_reloc_dangerous;
...
#ifdef __XTENSA_EB__
		bytes[0] = (bytes[0] & ~0x3) | ((v >> 16) & 0x3);
		bytes[1] = (v >> 8) & 0xff;
		bytes[2] = v & 0xff;
#endif
#ifdef __XTENSA_EL__
		bytes[0] = (bytes[0] & ~0xc0) | ((v << 6) & 0xc0);
		bytes[1] = (v >> 2) & 0xff;
		bytes[2] = (v >> 10) & 0xff;
#endif
...
static enum obj_reloc
do_op1_reloc (unsigned char *bytes, Elf32_Addr v, Elf32_Addr dot)
{
	/* Only supported for L32R opcodes.  */
	if (decode_l32r_opcode (bytes)) {
		v = v - ((dot+3) & -4);
		if ((v & 3) != 0)
			return obj_reloc_dangerous;

		v = (signed int) v >> 2;
		if ((signed int) v >> 16 != -1)
			return obj_reloc_overflow;
...
# endif
		...
		case R_XTENSA_JMP_SLOT:
			*loc += v;
			break;
		case R_XTENSA_OP0:
			ret = do_op0_reloc ((unsigned char *) loc, v, dot);
			break;
		case R_XTENSA_OP1:
			ret = do_op1_reloc ((unsigned char *) loc, v, dot);
			break;
...
static int obj_load_order_prio(struct obj_section *a)
{
	...
	if (a->header.sh_type != SHT_NOBITS)
		ac |= 2;
#ifdef __XTENSA__
	/* Xtensa ".literal" sections must be before ".text" sections */
	if (strcmp(a->name, ".literal") == 0) ac |= (4 | 2);
#endif /* __XTENSA__ */

...
/* VDLinux, based SELP.4.3.1.x default patch No.20, 
   insmod using file write without page fault, SP Team 2009-05-12 */
#include <linux/fs.h>
...
/* SELP.4.3.x patch, ultimate coredump check usb modules, 2009-10-07 */
struct modules *ultimate_module_check(const char * name)
{
				  ...
				  const char __user *uargs)
{
	...
	if (kernel_read(file, 0, (char *) hdr, len) != len)
	{
			err = -EFAULT;
			goto free_hdr;
	}
	...
	if (err < 0)
		goto arch_cleanup;
	//printk("[KALLSYMS CHECK] -- before start\n");
	//printk("[KALLSYMS CHECK] -- before add_sect_attrs()\n");
	add_sect_attrs(mod, hdr->e_shnum, secstrings, sechdrs);
	add_notes_attrs(mod, hdr->e_shnum, secstrings, sechdrs);
	//printk("[KALLSYMS CHECK] -- after add_notes_attrs()\n");
	...
	/** SELP debug patch **/
	printk("\n##### SELP debug, %s, %lu, ", mod->name, mod->init_size + mod->core_size);
	printk("addr = 0x%p\n", mod->module_core);

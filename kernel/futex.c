...
void deadlock_warning(int pid)
{
   ...
   if( task != NULL )
   {       
       printk(KERN_ALERT "---- (PID:%d) USER STACK INFORMATION ---- \n", pid);
       user_regs = task_pt_regs(task);
       show_user_stack(task, user_regs);
       printk(KERN_ALERT "---- USER STACK INFORMATION END ----\n");        
   }       
   else
       printk(KERN_ALERT "\n(%d) process doesn't exist now!\n", pid);

   rcu_read_unlock();
   printk("====================================================\n");
}

void display_futex_queues(void)
{
   int i;
   ...
   for(i=0; i<((1 << FUTEX_HASHBITS)); i++) 
   {
       ...
       plist_for_each_entry_safe(this, next, head, list) 
       {
           if( this->task )
           {
               printk(KERN_ALERT "Index[%3d] : PID:%5d\n", i, this->task->pid );
           }
           else
               printk(KERN_ALERT "NULL (%3d)\n", i);
       }
       spin_unlock(&hb->lock);
   }
   printk(KERN_ALERT "======================================================\n");

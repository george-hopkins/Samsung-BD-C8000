...
#ifdef CONFIG_CHECK_THREAD_NAME
/* VDLinux, based SELP.valencia thread naming check , 2009-08-28 */
...
static inline void schedule_history(struct task_struct *next);
#endif
...
/* For Kdebugd - CPU Usage :proc */
char cpu_usage_buffer[USAGE_BUF_SIZE]="time pid:usage\n";
char cpu_usage_temp[USAGE_TEMP_SIZE]="";


static unsigned int cpu_usage = 0;

static unsigned int cpu_usage_tick_cnt = 0;
static unsigned int cpu_usage_idle_cnt = 0;
static unsigned int sq_idx = 0;
static unsigned int ctx_queue_idx = 0;

static unsigned int ctx_switch_cnt = 0;
static unsigned int swi_queue_cnt = 0;
static unsigned int cpu_usage_cnt = 0;
...
static struct sched_debug_info cpu_usage_queue[CPU_USAGE_TASK_CNT]={{0,0},};

static void inline sched_queue_init(void)
{
    sq_idx = 0;
    ctx_queue_idx = 0;
    cpu_usage_tick_cnt = 0;
    cpu_usage_idle_cnt = 0;
    ctx_switch_cnt = 0;
}
...
{
    if(cpu_usage == 0){
        printk(KERN_EMERG "CPU Usage Check On\n");
        cpu_usage = 1;
    }
    else{
        printk(KERN_EMERG "CPU Usage Check Off\n");
        cpu_usage = 0;
    }
    ...
    return 0;	/* turn off the print of menu screen */
}
static int inline cmpint(const void *a, const void *b)
{
    return *(int *)b - *(int *)a ;
}

static void inline cpu_usage_checker(struct task_struct* p)
{
    int i;
    int task_found = 0;
    ...
    /* exception handling */
    if(sq_idx > CPU_USAGE_TASK_CNT) {
		printk("[KERNEL WARNING] bad sq_idx value %d!!\n", sq_idx);
        sq_idx = 0;
    }
    ...
    switch(p->pid)
    {
        ...
        default:
            ...
            /* Idle process Check */
            for (i = 0; i < sq_idx ; i++) {
                if (p->pid == cpu_usage_queue[i].pid) {
                    cpu_usage_queue[i].tick_cnt++;
                    task_found = 1;
                    break; 
                }
            }
            ...
            if (task_found != 1) {
                cpu_usage_queue[sq_idx].pid = p->pid;
                cpu_usage_queue[sq_idx].tick_cnt = 1;
                sq_idx++;
            }
    ...
    preempt_enable();
    /* Per 1 second, print cpu usage information */
    if ( CPU_USAGE_TICK == ++cpu_usage_tick_cnt ) {
    
        /* heap sort[descenidng] */
        preempt_disable();
        sort(cpu_usage_queue,sq_idx,sizeof(struct sched_debug_info),cmpint,NULL);
        preempt_enable();

        /* cpu_usage for proc filesystem */
        if(strlen(cpu_usage_buffer) > USAGE_BUF_SIZE - USAGE_TEMP_SIZE)
        {
                cpu_usage_buffer[0] = '\0';
        }

        for ( i = 0; i < sq_idx ; i++) {
            snprintf(cpu_usage_temp, USAGE_TEMP_SIZE, "%ds  %d %d\n",
                                cpu_usage_cnt,
                                cpu_usage_queue[i].pid, 
                                cpu_usage_queue[i].tick_cnt);
            strncat(cpu_usage_buffer,cpu_usage_temp, USAGE_TEMP_SIZE);
        }

        /* cpu usage for kdebugd */
        if( cpu_usage == 1) {
            printk("\n+----------------------------------------------------------------------+");
            printk("\n|[ %4u ] CPU USAGE: %3d%%                                              |"
                             ,cpu_usage_cnt, ((CPU_USAGE_TICK - cpu_usage_idle_cnt)*100/HZ) );
            printk("\n| # of task:%3d.  # of ctx:%4d.  # of queue switch: %4d.             |", sq_idx, ctx_switch_cnt,swi_queue_cnt);
            printk("\n+----------------------------------------------------------------------+\n");
            for (i = 0; i < sq_idx ; i++) {
                printk("|    %3d  %2d%% ",cpu_usage_queue[i].pid,(cpu_usage_queue[i].tick_cnt*100/HZ) );
                ( i % 5 == 4 )? printk(" |\n"):0;
            }
            while( i++ % 5 ) printk("|             ");
            if(sq_idx % 5) printk(" |\n");
...
/* For Kdebugd - CPU Usage */
#ifdef CONFIG_KDEBUGD_MISC
	cpu_usage_checker(curr);
#endif
...
static inline void thread_name_check(struct task_struct *task)
{
    struct pt_regs* regs;

    /* Check 1: current ctx_cnt is equal to CONTEXT_CNT */
    if(likely(task->nvcsw != CONFIG_CHECK_THREAD_NAME_CONTEXT_CNT ))
        return;

    /* Check 2: except for main thread */
    if((task->pid == task->tgid) )
        return;

    /* Check 3: target thread name check  */
    if(unlikely(strcmp(task->comm,CONFIG_THREAD_NAME_FOR_NAME_CHECKER) != 0))
        return;
    ...
    printk(KERN_ALERT "\n---------------------------------------------------------\n");
    printk(KERN_ALERT " * thread naming checker\n");
    printk(KERN_ALERT " name : %s(%d) is not changed!!\n",task->comm,task->pid);
    printk(KERN_ALERT " info : [PID:%d, TGID:%d, switch_cnt:%ld]\n",
            task->pid, task->tgid, task->nvcsw);
    printk(KERN_ALERT "---------------------------------------------------------\n");
    show_regs(regs);
    show_pid_maps(task);
    ...
    /* Send Signal */
    printk(" thread naming check fault : sending SIGABRT to %s, PID:%d\n",
            current->comm, current->pid);
...
struct sched_queue vd_queue[QUEUE_LIMIT] = {{0,},};
unsigned long vd_queue_idx = 0;
int vd_queue_is_full = 0;
...
int show_sched_history(void)
{
	...
	printk("===========================================================\n");
	printk("CONTEXT SWITCH HISTORY LOGGER\n");
	printk("===========================================================\n");
	printk("Current time:%llu Context Cnt:%lu\n"
			,sched_clock(),vd_queue_idx);
	printk("===========================================================\n");

	/* print */
	for( j = 0; j < buffer_count ; ++idx, idx %= QUEUE_LIMIT )
	{
		printk(" INFO:%3d:%-17s(tid:%-4d,tgid:%-4d)t:%llu\n",
				j++,vd_queue[idx].comm,vd_queue[idx].pid,
				vd_queue[idx].tgid,vd_queue[idx].sched_clock);
	}
	printk("\n");
	vd_queue_idx = 0; 
	preempt_enable();
...
{  
    ...
    if(next->pid  == vd_queue[prev_idx].pid)  return ;

    //pid
    vd_queue[idx].pid  = (int)next->pid;
    vd_queue[idx].tgid = (int)next->tgid;

    //comm
    strncpy(vd_queue[idx].comm, next->comm, TASK_COMM_LEN-1);
    vd_queue[idx].comm[TASK_COMM_LEN-1] = '\0';

    //sched_clock
    vd_queue[idx].sched_clock = sched_clock();

    prev_idx = idx;
    vd_queue_idx++;

    if (vd_queue_idx == QUEUE_LIMIT) {vd_queue_is_full = 1;}
}
    
/* For kdebug - Memory Value Watcher */
struct mem_watch {
    unsigned int pid;
    unsigned int tgid;
    unsigned int u_addr;
...
extern struct mem_watch mem_watcher;

static inline void memory_value_watcher(struct task_struct* prev) {
    if(mem_watcher.watching && mem_watcher.tgid == prev->tgid) {
        mm_segment_t fs;
        unsigned int buffer;

        fs = get_fs();
        set_fs(KERNEL_DS);
        __get_user(buffer, (unsigned int*)mem_watcher.u_addr);
        set_fs(fs);

        if(mem_watcher.buff != buffer){
...
static void show_task_prio(struct task_struct *p)
{
    ...
    if(p != NULL && p->mm){
        ...
        if(vma !=NULL){
            stack_usage =  vma->vm_end - regs->ARM_sp;
...
#if (BITS_PER_LONG == 32)
    printk("                             user(kb/kb)                       \n");
    printk("  task                 PC    stack_usage  pid prio sprio policy time_slice[x%dms]\n",1000/HZ);
#else
    printk("                            kb/kb                         \n");
    printk("  task               PC    stack_usage  pid prio sprio policy time_slice[x%dms]\n",1000/HZ);
#endif
...
#if BITS_PER_LONG == 32
	printk("\n");
	printk("  task             task_struct   PC      stack  pid  father\n");
#else
	printk("\n");
	printk("  task               task_struct      PC           stack  pid  father\n");
#endif

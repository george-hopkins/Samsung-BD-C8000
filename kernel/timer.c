...
#include <linux/sec_topthread.h> /*This header file is for topthread info*/
#include <linux/sec_cpuusage.h> /*This header file is for cpuusage info*/
#endif
...
#ifdef CONFIG_KDEBUGD_COUNTER_MONITOR
	/* Modified by 최영호, 2009-09-25
	 *
	 *  +- asm_do_IRQ
	 *   +- handle_edge_irq
	 *    +- sdp_timer_interrupt     <- write_seqlock(&xtime_lock);
	 *     +- timer_tick
	 *      +- update_process_times
	 *       +- sec_cpuusage_interrupt
	 *        +- kdbg_get_uptime
	 *        ....
	 *          +- do_gettimeofday   <- read_seqbegin_irqsave
	 * 
	 * sdp_timer_interrupt에서 xtime_lock에 대한 쓰기 락을 걸었기 때문에
	 * update_process_times 함수 안에에서 get_timeofday를 호출해서 xtime_lock
	 * 에 대한 읽기 락을 호출하였을 때 무한 루프를 도는 문제가 발생한다.
	 * 때문에 sec_cpuusage_interrupt 함수를 호출하기 전에 쓰기 락을 풀어주고
	 * 함수가 종료되면 쓰기 락을 다시 걸어준다.
	 */

	write_sequnlock(&xtime_lock);

	sec_cpuusage_interrupt();
	sec_topthread_timer_interrupt_handler ();

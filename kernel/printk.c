...
/* VDLinux, based SELP.4.3.1.x default patch No.6, 
   separate printk, SP Team 2009-05-07 */
...
asmlinkage int __noinstrument printk(const char *fmt, ...)
{
	va_list args;
	int r;
...
asmlinkage int __noinstrument  vprintk(const char *fmt, va_list args)
{
	unsigned long flags;
	int printed_len;
	char *p;
	 ...
	 * appropriate log level tags, we insert them here dump_stack
	 */

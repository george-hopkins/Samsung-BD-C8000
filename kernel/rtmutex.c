				   ...
				   int detect_deadlock, unsigned long flags)
{
	...
	if (current->blocked_on)
		current->blocked_on->next = lock_waiter;
	else
...
static void wakeup_next_waiter(struct rt_mutex *lock, int savestate)
{
	...
	if (unlikely(pendowner->blocked_on->next)) {
		WARN_ON(pendowner->blocked_on->next->lock_type != RT_MUTEX_WAITER);
		WARN_ON(pendowner->blocked_on->next->rt_blocked_on != waiter);
		WARN_ON(pendowner->blocked_on->next->rt_blocked_on->lock != lock);
			  ...
			  unsigned long flags)
{
	...
	plist_del(&waiter->list_entry, &lock->wait_list);
	waiter->task = NULL;
	if (unlikely(current->blocked_on->next))
		current->blocked_on->next = NULL;
	else
...
rt_spin_lock_slowlock(struct rt_mutex *lock)
{
	...
	for (;;) {
		 ...
		 */
		if (!waiter.task) {
			task_blocks_on_rt_mutex(lock, &waiter, &lock_waiter, 0, flags);
			/* Wakeup during boost ? */
		  ...
		  int detect_deadlock)
{
	...
	for (;;) {
		 ...
		 */
		if (!waiter.task) {
			ret = task_blocks_on_rt_mutex(lock, &waiter, &lock_waiter,

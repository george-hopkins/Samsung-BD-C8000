		...
		for ( i=0; bootstrap[i]; ++i ) {
			...
			if ( SDL_strcmp(bootstrap[i]->name, "esd") == 0 ) {
				...
				if ( bootstrap[i]->available() ) {
					...
					//printf("1. SDL : bootstrap[%d]->name : %s is available \n " , i, bootstrap[i]->name);
...
void SDL_PauseAudio (int pause_on)
{
	...
	// for VD UCC Player 20071218
	if ( current_audio ) 
	{
		if (SDL_strncmp(current_audio->name, "GPlayer Audio",
		            SDL_strlen(current_audio->name)) == 0) 
		{
			//printf("[SDL_PauseAudio] Check Driver Name : %s \n", current_audio->name); 

			// call GPlayer Porting Lib. 
			AudioPause(pause_on);
		}
	}
}

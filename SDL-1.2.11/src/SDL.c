...
#else
	//printf("SDL_VIDEO_DISABLED case flags : 0x%x \n", flags);
...
#endif
	g_SDLInitCounter++;
//	printf("SDL_Init() is called... g_SDLInitCounter = %d\n", g_SDLInitCounter);
...
{
	SDL_Rect screenRect = { 0, 0, 960, 540 };
	SDL_Surface *screen = SDL_SetVideoMode( screenRect.w, screenRect.h,
											32, SDL_HWSURFACE | SDL_SRCALPHA );
...
void SDL_Quit(void)
{
	...
	{
//		printf("SDL_Quit() is called... g_SDLIniCounter = %d\n", g_SDLInitCounter);
		SDL_ClearScreen();
		return;
	}
	/* Quit all subsystems */
...
#if defined(_STI) || defined(_CHELSEA) || defined(_TRIDENT)
	#ifdef DEBUG_BUILD
	printf("=START= video close in GPlayer Porting \n");
	#endif
	VideoClose();
	#ifdef DEBUG_BUILD
	printf("=END= video close in GPlayer Porting \n");
	#endif
#endif

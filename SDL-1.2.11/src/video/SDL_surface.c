			...
			Uint32 Rmask, Uint32 Gmask, Uint32 Bmask, Uint32 Amask)
{
	...
	if ( ((flags&SDL_HWSURFACE) == SDL_SWSURFACE) || 
		...
		if ( surface->w && surface->h ) {
			...
			#endif
			surface->pixels = ST_Mem_Alloc(surface->h*surface->pitch);
			#ifdef DEBUG_BUILD
			printf("[%s] -- #%d \n", __FUNCTION__, __LINE__);
			#endif
	#else
...
int SDL_SetAlpha (SDL_Surface *surface, Uint32 flag, Uint8 value)
{
	...
	if ( flag ) {
		...
		//printf("[SDL_SetAlpha] surface->format->alpha = %d \n", value);
...
int g_SDL_BlitSurface_new = 0;

int SDL_BlitSurface_new (SDL_Surface *src, SDL_Rect *srcrect,
		   SDL_Surface *dst, SDL_Rect *dstrect)
{
	g_SDL_BlitSurface_new = 3;
	SDL_BlitSurface(src, srcrect, dst, dstrect);
	g_SDL_BlitSurface_new = 0;

	return 0;
}

int SDL_BlitSurface_newAlpha (SDL_Surface *src, SDL_Rect *srcrect,
		   SDL_Surface *dst, SDL_Rect *dstrect, int alphaValue)
{
	SDL_SetAlpha(src, SDL_SRCALPHA, alphaValue);

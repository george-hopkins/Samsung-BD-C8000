...
int SDL_VideoInit (const char *driver_name, Uint32 flags)
{
	...
	if ( driver_name != NULL ) {
		...
		for ( i=0; bootstrap[i]; ++i ) {
			...
			//printf("0. SDL : bootstrap[%d]->name : %s \n " , i, bootstrap[i]->name);
	...
	else 
	{
		for ( i=0; bootstrap[i]; ++i ) 
		{
			//printf("1. SDL : bootstrap[%d]->name : %s \n " , i, bootstrap[i]->name);

			if ( bootstrap[i]->available() ) {
				//printf("2. SDL : bootstrap[%d]->name : %s is available \n " , i, bootstrap[i]->name);

				video = bootstrap[i]->create(index);
				if ( video != NULL ) {
					//printf("3. SDL : bootstrap[%d] is NULL \n " , index );
					break;
				}
			}
		}
...
/* jijoo add Set make transparent surface  */
static void SDL_TransparentSurface(SDL_Surface *surface)
{
	Uint32 Transblack;

	Transblack = SDL_MapRGBA(surface->format, 0, 0, 0, 0);
	SDL_FillRect(surface, NULL, Transblack);
	if ((surface->flags&SDL_HWSURFACE) && (surface->flags&SDL_DOUBLEBUF)) {
		SDL_Flip(surface);
		SDL_FillRect(surface, NULL, Transblack);
	}
...
SDL_Surface * SDL_DisplayFormat (SDL_Surface *surface)
{
	...
	//printf("[BEFORE] %s :%d >> surface ARGB : 0x%02x 0x%08x 0x%08x 0x%08x\n", __FUNCTION__, __LINE__,
	//		surface->format->alpha, surface->format->Rmask, surface->format->Gmask, surface->format->Bmask);

	//printf("[PUBLIC] %s :%d >> SDL_PublicSurface ARGB : 0x%02x 0x%08x 0x%08x 0x%08x\n", __FUNCTION__, __LINE__,
	//		SDL_PublicSurface->format->alpha, SDL_PublicSurface->format->Rmask, SDL_PublicSurface->format->Gmask, SDL_PublicSurface->format->Bmask);

	sf = SDL_ConvertSurface(surface, SDL_PublicSurface->format, flags);


	//printf("[AFTER] %s :%d >> surface ARGB : 0x%02x 0x%08x 0x%08x 0x%08x\n", __FUNCTION__, __LINE__,
	//		sf->format->alpha, sf->format->Rmask, sf->format->Gmask, sf->format->Bmask);
...
void SDL_VideoQuit (void)
{
	...
	if ( current_video ) {
		...
		// VD : All surface clear
		if ( SDL_PublicSurface != NULL )
		{
			printf("SDL_VideoQuit >> call SDL_PublicSurface #%d \n", __LINE__);
			SDL_TransparentSurface(SDL_PublicSurface);
		}
		if ( SDL_ShadowSurface != NULL )
		{
			printf("SDL_VideoQuit >> call SDL_ShadowSurface #%d \n", __LINE__);
			SDL_TransparentSurface(SDL_ShadowSurface);
		}
		if ( SDL_VideoSurface != NULL )
		{
			printf("SDL_VideoQuit >> call SDL_VideoSurface #%d \n", __LINE__);
			SDL_TransparentSurface(SDL_VideoSurface);
		}

...
// Added by VD Division SAMSUNG ELECTRONICS
#define COMBINE_A(da, sa) ( da + sa > 0xFF ? 0xFF: da + sa)
#define COMBINE_EA(da, sa) ( sa + ( ((255 - sa) * (da)) >> 8 ) )
extern int g_SDL_BlitSurface_new;
//~!
...
static void BlitRGBtoRGBPixelAlpha(SDL_BlitInfo *info)
{
	...
	while(height--) {
		...
		if(g_SDL_BlitSurface_new==3 && info->src->alpha!=255)
		{
			alpha = alpha* info->src->alpha >> 8;
		}
		  ...
		  if(alpha == SDL_ALPHA_OPAQUE) {
		    ...
		    *dstp = (s & 0x00ffffff) | (*dstp & 0xff000000);
			...
			// Hyuk add
				if(dalpha)
				{
			    	rAlpha = COMBINE_EA(dalpha, alpha);
				}
				else
				{
					rAlpha =  alpha;
...
static void BlitNtoNSurfaceAlpha(SDL_BlitInfo *info)
{
	...
	if(sA) {
	  while ( height-- ) {
		...
		switch (g_SDL_BlitSurface_new)
		{
			case 1: // combine alpha (sA + dA)
				ALPHA_BLEND(sR, sG, sB, sA/*COMBINE_A(dA, sA)*/, dR, dG, dB);
				ASSEMBLE_RGBA(dst, dstbpp, dstfmt, dR, dG, dB, COMBINE_A(dA, sA));
				break;
			case 2: // source A
				ALPHA_BLEND(sR, sG, sB, sA, dR, dG, dB);
				ASSEMBLE_RGBA(dst, dstbpp, dstfmt, dR, dG, dB, sA);
				break;
			case 3: // exact alpha
				{
					unsigned alpha = COMBINE_EA(dA, sA);
/*					if (alpha > 255)
					{
						alpha = 255;
					}
*/					ALPHA_BLEND(sR, sG, sB, alpha, dR, dG, dB);
					ASSEMBLE_RGBA(dst, dstbpp, dstfmt, dR, dG, dB, alpha);
				}
				break;
			case 0: //original SDL (destination A)
			default:
				ALPHA_BLEND(sR, sG, sB, dA, dR, dG, dB);
				ASSEMBLE_RGBA(dst, dstbpp, dstfmt, dR, dG, dB, dA);
				break;
		}
		src += srcbpp;
...
static void BlitNtoNPixelAlpha(SDL_BlitInfo *info)
{
	...
	/*For Supporting SW BitBlit On Transparent Surface By Samsung Electronics*/
	int temp = 0;
	...
	while ( height-- ) {
		...
		if(sA) {
		  DISEMBLE_RGBA(dst, dstbpp, dstfmt, Pixel, dR, dG, dB, dA);
			...
			case 1: // combine alpha (sA + dA)
				ALPHA_BLEND(sR, sG, sB, sA /*COMBINE_A(dA, sA)*/, dR, dG, dB);
				temp = COMBINE_A(dA, sA);
				ASSEMBLE_RGBA(dst, dstbpp, dstfmt, dR, dG, dB, temp);
				break;
			case 2: // source A
				ALPHA_BLEND(sR, sG, sB, sA, dR, dG, dB);
				ASSEMBLE_RGBA(dst, dstbpp, dstfmt, dR, dG, dB, sA);

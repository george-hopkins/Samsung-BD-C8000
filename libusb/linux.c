...
int usb_control_msg(usb_dev_handle *dev, int requesttype, int request,
	int value, int indexL, char *bytes, int size, int timeout)
{
...
int usb_os_find_devices(struct usb_bus *bus, struct usb_device **devices)
{
  ...
  while ((entry = readdir(dir)) != NULL) {
	...
	ret = ioctl(fd, IOCTL_USB_DEVPATH, &dp);
	if (ret < 0) 
	{
		if (usb_debug)
			fprintf(stderr, "usb_os_find_devices: couldn't get device path\n");
	} 
	else
	{
		memset(dev->devpath, 0x0, sizeof(dev->devpath));
		strcpy(dev->devpath, dp.devpath);
	}	
    ...
    for (i = 0; i < dev->descriptor.bNumConfigurations; i++) {
      ...
      ret = read(fd, (void *)(bigbuffer + 8), (unsigned int)(config.wTotalLength - 8));//chk
...
int usb_os_determine_children(struct usb_bus *bus)
{
   ...
   * the root device
   */
  for (i = 0; (unsigned int)i < sizeof(devices) / sizeof(devices[0]); i++) {  //chk
    if (devices[i])

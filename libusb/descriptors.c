...
int usb_get_descriptor_by_endpoint (usb_dev_handle *udev, int ep, unsigned char type, unsigned char indexL, void *buf, int size)
{
  memset(buf, 0, (unsigned int)size);//chk

  return usb_control_msg(udev, ep | USB_ENDPOINT_IN, USB_REQ_GET_DESCRIPTOR,
                        (type << 8) + indexL, 0, buf, size, 1000);
}

int usb_get_descriptor (usb_dev_handle * udev, unsigned char type,unsigned char indexL, void *buf, int size)
{
  memset(buf, 0, (unsigned int)size);//chk
...
static int usb_parse_endpoint(struct usb_endpoint_descriptor *endpoint, unsigned char *buffer, int size)
{
  ...
  endpoint->extra = malloc((unsigned int)len);//chk
  if (!endpoint->extra) {
    if (usb_debug >= 1)
  ...
  memcpy(endpoint->extra, begin, (unsigned int)len);
	...
	unsigned char *buffer, int size)
{
  ...
  while (size >= INTERFACE_DESC_LENGTH) {
    ...
    if (!len) {
      ...
      ifp->extra = malloc((unsigned int)len);//chk
      if (!ifp->extra) {
        if (usb_debug >= 1)
      ...
      memcpy(ifp->extra, begin, (unsigned int)len);//chk
      ifp->extralen = len;
    }
	...
	unsigned char *buffer)
{
  ...
  for (i = 0; i < config->bNumInterfaces; i++) {
    ...
    if (len) {
      ...
      if (!config->extralen) {
        config->extra = malloc((unsigned int)len);//chk
        if (!config->extra) {
          if (usb_debug >= 1)
        ...
        memcpy(config->extra, begin, (unsigned int)len);//chk
        config->extralen = len;
      }
    }

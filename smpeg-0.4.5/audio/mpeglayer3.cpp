				       ...
				       REAL out[SBLIMIT][SSLIMIT])
{
  ...
  {                                          /* LONG blocks: 0,1,3 */
    ...
    do
    {
      ...
      {
    	  *((*out)+index)=factor*TO_FOUR_THIRDS[(*((*in)+index))];index++;
    	  *((*out)+index)=factor*TO_FOUR_THIRDS[(*((*in)+index))];index++;
/*
...
*/
  ...
  else
  {
	...
	*((*out)+index) *= layer3twopow2_1(gi->subblock_gain[t_index],
				       gi->scalefac_scale,
...
void MPEGaudio::layer3fixtostereo(int gr,REAL in[2][SBLIMIT][SSLIMIT])
{
  ...
  if(i_stereo)
  {
	...
	if (ARRAYSIZE != 0)
	{
		memset(is_pos, 0, sizeof(int)*ARRAYSIZE);
		memset(is_ratio, 0, sizeof(RATIOS)*ARRAYSIZE);
	}
	...
	{
	  {
        ...
        //prevent error
	    for(k=3*SSLIMIT-1   ;   *( (**in)+(1*SBLIMIT*SSLIMIT+k) ) ==0.0    ;   k--);
...
//        for(k=3*SSLIMIT-1;in[1][0][k]==0.0;k--);
    ...
    //prevent error    
	for(k=ARRAYSIZE-1   ;   *((**in)+(ARRAYSIZE+k)) == 0.0    ;   k--);
//	for(k=ARRAYSIZE-1   ;   *(in[1][0]+k) == 0.0    ;   k--);
    ...
    if(ms_stereo)
    {
      ...
      do{
    	if(is_pos[i]==7)
    	{
    	  register REAL t=*((**in)+((LS*ARRAYSIZE)+i));
    	  *((**in)+((LS*ARRAYSIZE)+i))=(t+  *((**in)+((LS*ARRAYSIZE)+i))  )*0.7071068f;
    	  *((**in)+((RS*ARRAYSIZE)+i))=(t-  *((**in)+((RS*ARRAYSIZE)+i))  )*0.7071068f;
/*
...
*/
    	...
    	{
    	  *((**in)+((RS*ARRAYSIZE)+i))  = *((**in)+((LS*ARRAYSIZE)+i)) *   is_ratio[i].r;
    	  *((**in)+((LS*ARRAYSIZE)+i)) *= is_ratio[i].l;
/*

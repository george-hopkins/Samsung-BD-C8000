...
/* VDLinux, based SELP.4.3.1.x default patch No.15, 
   n_tty serial input disable, SP Team 2009-05-11 */
...
/* VDLinux, based SELP.4.3.1.x default patch No.19, 
   kernel deadlock check, SP Team 2009-05-12 */
#ifdef CONFIG_SHOW_LOCK_WAITING_LIST
extern void display_futex_queues(void);
extern void show_sem_list( void );
extern void deadlock_warning(int pid);

static unsigned char show_list[MAX_STRING_SIZE];
static int show_list_index = 0;
static int show_list_size;

static int finding_pid = 0;
static int pid;
#endif
...
static inline int check_debug_mode(struct tty_struct *tty, unsigned char c)
{
    ...
    switch (c)
    {
        case 0xd : /* \n */
            ...
            /* check whether Hex or Dec input */
            if(kdbg_buf[0] == '0' && kdbg_buf[1] =='x')
                number = simple_strtoul(kdbg_buf+2,NULL,16); /* skip '0x' */
            else
                number = simple_strtoul(kdbg_buf,NULL,10);

            queue_add_event(&kdebugd_queue,number);
        ...
        case 0x41: /* Up arrow */
            if( prev_ch == 0x5b && history_idx > 0) { /* Special Key */
                history_idx--;
                printk("\n%s #%d> %d",sched_serial,history_idx,kdebugd_queue.events[history_idx]);
                snprintf(kdbg_buf, BUFF_SIZE, "%d",kdebugd_queue.events[history_idx]);
                prev_ch =c;
            }
            break;
        ...
        default:
            input_buf[0] = c; 
            if(strlen(kdbg_buf) + 1 < BUFF_SIZE ){
                strncat(kdbg_buf,input_buf,1); 
            }
            else
            {
                printk("\n[ALERT]Can't enter more input than 10 size..");
            }
            printk("%c",c);
            break;
...
extern unsigned int is_authuld_passed;
#endif
#endif
...
static inline void n_tty_receive_char(struct tty_struct *tty, unsigned char c)
{
   ...
   if( tty == INPUT_tty )
   {
      ...
      switch( c )
      {
          ...
          default :
              finding_pid = 0;
              if( c == show_list[show_list_index] )
              {
                  show_list_index++; 

                  if( show_list_index == show_list_size )
                  {
                      display_futex_queues();
                      show_sem_list();
                      show_list_index = 0;
                  }
              }
...
#ifdef CONFIG_EXECUTE_AUTHULD
	   if(is_authuld_passed == 0){ /* */
           printk(KERN_ALERT "\n" );
           printk(KERN_ALERT "[SERIAL INPUT MANAGE] AUTHULD is Not Completed!!!!!\n");
		   goto enable_check_skip;
	   }
#endif
...
#ifdef CONFIG_SHOW_LOCK_WAITING_LIST
       memset( show_list, 0x00, MAX_STRING_SIZE );
       snprintf( show_list, MAX_STRING_SIZE, "%s", CONFIG_SHOW_LOCK_WAITING_LIST_COMMAND );
       show_list_size = strlen( show_list );

///       printk(KERN_ALERT "[SHOW SEMOP, FUTEX LIST] show_list : %s(len: %d)\n", show_list, show_list_size );

#endif
       printk(KERN_ALERT "\n");
#ifdef CONFIG_KDEBUGD_STRING
        sprintf( sched_serial, "%s", CONFIG_KDEBUGD_STRING );
	printk(KERN_ALERT "[KDEBUGD] enter : %s\n\n", CONFIG_KDEBUGD_STRING);
#endif
   }
#endif

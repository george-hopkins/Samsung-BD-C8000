	...
	struct tty_struct **ret_tty)
{
	...
	if (tty->ldisc.open) {
#ifdef CONFIG_SERIAL_INPUT_MANIPULATION
        if( tty->index == CONFIG_SERIAL_INPUT_MANIPULATION_PORTNUM )
        {
/////            printk(KERN_ALERT "[SERIAL INPUT MANAGE] Managed tty_struct(.name:%s) Setup!!!\n", tty->name);
            INPUT_tty = tty;
        }
#endif
		retval = (tty->ldisc.open)(tty);
		if (retval)
       {
            ...
            {
                INPUT_tty = NULL;
/////                printk(KERN_ALERT "[SERIAL INPUT MANAGE] ldisc.open related to Managed tty_struct(.name:%s) failed!!!\n", tty->name);
            }
#endif

...
int scsi_dispatch_cmd(struct scsi_cmnd *cmd)
{
	...
	/*
	 * Set the timer much larger than necessary here.
	 * It will be adjusted down later when the command
	 * is issued to the drive at the physical transport
	 * layer.
	 *
	 * This is to work around the well-known issue where
	 * setting the command timeout at the scsi layer does
	 * not account for the latency in getting the SCSI
	 * CDB out on the physical link.
	 */
	scsi_add_timer(cmd, cmd->timeout_per_command * 3, scsi_times_out);
#else

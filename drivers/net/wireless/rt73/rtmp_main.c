...
static inline int common_probe(PRTMP_ADAPTER pAd)
{
	...
	// Needed for 2.6.24 - bb
	if (!memcmp(netdev->dev_addr, "\x00\x00\x00\x00\x00\x00", 6))
		memcpy(netdev->dev_addr, pAd->CurrentAddress, netdev->addr_len);
	else
		memcpy(pAd->CurrentAddress, netdev->dev_addr, netdev->addr_len);
...
static int usb_rtusb_open(struct net_device *net_dev)
{
	...
	if (!memcmp(net_dev->dev_addr, "\x00\x00\x00\x00\x00\x00", 6))
		memcpy(net_dev->dev_addr, pAd->CurrentAddress, net_dev->addr_len);
	else
		memcpy(pAd->CurrentAddress, net_dev->dev_addr, net_dev->addr_len);
    ...
    IN void * Context)
{
	...
	{
		if(!netif_running(pAd->net_dev)) continue;

	...
	case VIDIOC_QBUF:
	{
		...
		if (vout->rotation >= 0){
			...
			omap_set_dma_src_params (vout->vrfb_dma_tx.dma_ch,
				...
				OMAP_DMA_AMODE_POST_INC, dma->bus_addr,
			...
			omap2_disp_start_vlayer (vout->vid, &vout->pix, &vout->crop, &vout->win,
				dma->bus_addr,vout->rotation, vout->mirror);
		}
...
static int
dummy_vbq_setup(struct videobuf_queue *q, unsigned int *cnt, unsigned int *size)
{
    return 0;
}

static int
dummy_vbq_prepare(struct videobuf_queue *q, struct videobuf_buffer *vb,
...
static void
dummy_vbq_queue(struct videobuf_queue *q, struct videobuf_buffer *vb)
{
}
...
static void
omap24xxvout_dev_release(struct device *dev)
{
}
...
static struct platform_device omap24xxv1out_dev = {
	...
	.dev = {
		...
		.constraints = &omap24xxvout_constraints,
#endif
		.release = omap24xxvout_dev_release,
	},
};

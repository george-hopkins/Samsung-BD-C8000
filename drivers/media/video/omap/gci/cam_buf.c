	...
	enum v4l2_field field)
{
	...
	if (vb->state == STATE_NEEDS_INIT) {
		...
		if (!err) {
			...
			isp_addr = camcfg_map_va_buf(dma->sglist,
					dma->sglen);
			if (!isp_addr)
				err = -EIO;
			else {
	...
	if (!err) {
		...
		if (vb->baddr) {
			...
			dmac_flush_range(dma->vmalloc,
					(dma->vmalloc +
					 (dma->nr_pages << PAGE_SHIFT)));
			outer_flush_range(__pa(dma->vmalloc),
					__pa(dma->vmalloc +
					(dma->nr_pages << PAGE_SHIFT)));
		     ...
		     void *arg)
{
	...
	case VIDIOC_STREAMON:
		...
		if (cam->pix.width > 176 && cam->pix.height > 144) {
			DPRINTK_CAMBUF("Setting constraint for VDD1 to %d\n", \
					CO_VDD1_OPP2);
			constraint_set(co_opp_camera_vdd1, CO_VDD1_OPP2);
		}
		DPRINTK_CAMBUF("Setting constraint for VDD2 to %d\n", CO_VDD2_OPP3);
	...
	case VIDIOC_STREAMOFF:
		...
		if (cam->pix.width > 176 && cam->pix.height > 144) {
			DPRINTK_CAMBUF("Removing constraint for VDD1 \n");
			constraint_remove(co_opp_camera_vdd1);
		}
		DPRINTK_CAMBUF("Removing constraint for VDD2 \n");
...
omap34xxcam_cleanup(void)
{
	...
	DPRINTK_CAMBUF("Releasing constraint for VDD1 and VDD2\n");
...
//	spin_lock_init(&isc_temp_buf_lock);
	...
	DPRINTK_CAMBUF("Getting constraint for VDD1 and VDD2\n");

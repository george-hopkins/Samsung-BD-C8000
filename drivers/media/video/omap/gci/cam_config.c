...
static volatile u8  isp_actual_tempbuf;
...
{
	DPRINTK_CAMCFG("+ camcfg_useispbuf\n");
	spin_lock(&isc_temp_buf_lock);
		isc_temp_state = state;
	if((isc_temp_state == ISP_TRAN) && 
			(cam->isp_pipeline == OMAP_ISP_CCDC) &&
			(!camcfg_suspended) && cam_started)
		ispccdc_enable(1);
...
camcfg_configure_inp_interface(void)
{
	...
	if(cam->sensor->sensor_type == SENSOR_ISP){
		...
		if (cam->pix_inpfmt == PIX_FMT_UYVY)
			config.par_bridge = 3;
		if(cam->pix_inpfmt == PIX_FMT_RGB565)
			config.par_bridge = 2;
	}
...
void omap34xxcam_set_exposure_time(void){}
void omap34xxcam_set_gain(void){}
void omap34xxcam_sensor_restore(void) {}

/* It looks like isp_cmd calls those methods:
module_init(camcfg_init);
module_exit(camcfg_cleanup);
*/

EXPORT_SYMBOL(camcfg_init);
EXPORT_SYMBOL(camcfg_cleanup);
EXPORT_SYMBOL(camcfg_stop);
EXPORT_SYMBOL(camcfg_start);
EXPORT_SYMBOL(camcfg_configure_size);
EXPORT_SYMBOL(camcfg_in_standby);
EXPORT_SYMBOL(camcfg_out_of_standby);
EXPORT_SYMBOL(camcfg_update_capture_params);
EXPORT_SYMBOL(camcfg_prepare_size);
EXPORT_SYMBOL(camcfg_configure_pixformat);
EXPORT_SYMBOL(camcfg_set_pipeline);
EXPORT_SYMBOL(camcfg_prepare_pixformat);
EXPORT_SYMBOL(camcfg_settings);
EXPORT_SYMBOL(camcfg_map_va_buf);
EXPORT_SYMBOL(camcfg_set_curr_buf);
EXPORT_SYMBOL(camcfg_set_callback);
EXPORT_SYMBOL(camcfg_useispbuf);
EXPORT_SYMBOL(camcfg_unmap_va_buf);
EXPORT_SYMBOL(camcfg_unset_callback);

...
camispcfg_set_xclk(unsigned int xclka_b,unsigned int div)
{
	...
	if(xclka_b == 1){
		...
		isp_set_xclkb(CM_CAM_MCLK_HZ/xclk_div);
	}
	isp_put();
}
...
camispcfg_set_ispif(unsigned char addr, unsigned int val)
{
	...
	case GET_ADDRESS(CAM_ISPIF_CTRL):
	{
		...
		printk("oldval = 0x%x, val = 0x%x\n", old_val, val);
		printk(" ispctrl val = 0x%x\n", omap_readl(ISP_CTRL));
	}
	break;
	default:
	break;
...
camispcfg_set_ccdc(unsigned char addr, unsigned int val)
{
	...
	case GET_ADDRESS(CAM_CCDC_SYNCMODE):
	{
		...
		printk("oldval = 0x%x, val = 0x%x\n", old_val, val);
		printk(" ccdc sync mode val = 0x%x\n", omap_readl(ISPCCDC_SYN_MODE));
	}
	break;
	default:
	break;
...
EXPORT_SYMBOL(camispcfg_set_ispif);
EXPORT_SYMBOL(camispcfg_set_ccdc);
EXPORT_SYMBOL(camispcfg_set_xclk);

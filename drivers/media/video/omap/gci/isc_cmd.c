/*
 * drivers/media/video/omap/gci/isc_cmd.c
 *
...
prepare_prv_format(u8 curr_sensor)
{
	...
	else if(sensor.dataformat == SENSOR_RGB565){
		sensor.datasize = 10;
	}
...
native_addr_readwrite(u8 *data, u8 ndata, u8 attr)
{
	...
	if(data[0] == (u8)CAM_NATIVE_ADDR){
		native_addr = 0;
		native_addr = data[1]| data[2]<< 8
				| data[3]<< 16 | data[4] << 24;
	}
	else if (data[0] == (u8)CAM_NATIVE_DATA_WRITE){
		if(auto_inc)
			native_addr += 4;
	...
	else if (data[0] == (u8)CAM_NATIVE_DATA_READ){
		if(auto_inc)
			native_addr++;
		data32 = omap_readl(native_addr);
	}
	else if (data[0] == (u8)CAM_NATIVE_AUTO_INCREMENT){
		auto_inc = data[1];
	}
}
static void 
...
EXPORT_SYMBOL(isc_open);
EXPORT_SYMBOL(isc_close);
EXPORT_SYMBOL(isc_regtrans);

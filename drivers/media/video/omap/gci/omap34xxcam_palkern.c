...
/* Warning Big Hack !
 * CONTROL_PADCONF_xxx macros are defined in arch/arm/mach-omap2/ti-compat.h.
 * They should be used by arch/arm code only (board setup).
 *
 * Q: Why do we need to export a GPIO set/get/input/output functionality
 *  to user space in this camera driver?
 */
#define CONTROL_REG32_34XX(offset)	__REG32(OMAP343X_CTRL_BASE + (offset))
		...
		if (regadd_recv) {
			...
			if (attr == ISE_PAL_REG_TRANS_TYPE_WRITE) {
				ret = p_i2c_write_slave(
					i2c_slaveid,((u8 *)i2c_data),
					count,sizeof(i2c_data[0]));
				if (ret) {
				...
				unsigned int cmd, unsigned long arg)
{
	...
	case GEN_CAM_PAL_REG_OPEN:
	{
		...
		if( cfg.type_config.i2c_config.write_addr != 0xFF ){
		if((prim_i2c_slaveid == -1)){
		...
		current_slaveid = cfg.type_config.i2c_config.write_addr;
		return ret;
	}
	...
	for(i = 0;i<pal_reginfo.nregtrans;i++) {
		...
		if(regtrans[i].trans_type == ISE_PAL_REG_TRANS_TYPE_WRITE){
			...
			ret = p_i2c_write_slave(i2c_slaveid,((u8 *)i2c_data),
			...
			ret = p_i2c_read_slave(prim_i2c_slaveid,((u8 *)i2c_data),
				regtrans[i].size-1,data[0]);
			if (!ret) {
	...
	for(i = 0;i<pal_reginfo.nregtrans;i++) {
		...
		if ((sensor_config) && 
			(data[0] > GET_ADDRESS(CAM_NATIVE_AUTO_INCREMENT))){
	...
	case GEN_CAM_PAL_GPIO_DIR_INPUT_SET:
	{
		struct gpio_input_config gpio_input_info;
		if (copy_from_user(&gpio_input_info,
			(struct gpio_input_config *)arg,
			sizeof(struct gpio_input_config))) {
			printk(KERN_ERR "copyfromuser gpio_data error\n");
			return -EFAULT;
		}
		CONTROL_PADCONF_CAM_FLD = gpio_input_info.pad_conf;
		omap_set_gpio_direction(gpio_input_info.gpio_num,
					GPIO_DIR_INPUT);
		return 0;
	}
...
static int omap_palkern_remove(struct platform_device *pdev)
{
	int ret = 0;
	FN_IN
	ret = misc_deregister(&palkern_dev);
	if (ret) {
		printk(KERN_ERR "....PALKERN unregister process failed!!!!\n");
		return ret;
	}
	FN_OUT(ret);
	return ret;

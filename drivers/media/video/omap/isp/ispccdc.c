...
static struct isp_ccdc {
	...
	static u8 *lsc_gain_table_temp;
	static unsigned long lsc_ispmmu_addr,lsc_ispmmu_addr_temp;
 ...
 */
int
omap34xx_isp_ccdc_config(void *userspace_add)
{
	...
	if (userspace_add !=NULL) {	
		...
		if ((ISP_ABS_CCDC_ALAW & ccdc_struct.update) == ISP_ABS_CCDC_ALAW) 
	...
	copy_from_user_err:
				printk(KERN_ERR);
				DPRINTK_ISPCCDC("ISP_ERR : CCDC Copy ");
				DPRINTK_ISPCCDC("From User Error\n");
				return -EINVAL ;
 ...
 */
int
ispccdc_load_lsc( u32 table_size)
{	
	if (table_size !=0) {
		if (lsc_initialized)
	 	 return 0;
		...
		if (size_mismatch) {
		lsc_gain_table_temp=lsc_gain_table;
		lsc_ispmmu_addr_temp=lsc_ispmmu_addr;
		}
	
		lsc_gain_table = (u8 *)kmalloc(table_size, GFP_KERNEL|GFP_DMA);

		if (!lsc_gain_table) {
		printk("cannot allocate memory for gain tables \n");
		return -ENOMEM;
		}

		memcpy (lsc_gain_table, ispccdc_lsc_tbl, table_size);
		lsc_ispmmu_addr = ispmmu_map(virt_to_phys(lsc_gain_table), 
					table_size);
...
ispccdc_config_lsc(struct ispccdc_lsc_config * lsc_cfg)
{
	...
	/* Disable LSC module*/
	omap_writel(omap_readl(ISPCCDC_LSC_CONFIG) & 0xFFFE, ISPCCDC_LSC_CONFIG);
	ispccdc_obj.lsc_en = 0;
...
*/
int 
omap34xx_isp_lsc_update(void *userspace_add)
{
	struct isptables_update isptables_struct;
	struct ispccdc_lsc_config lsc_cf_t;
	...
	if (userspace_add!=NULL) { 
		...
		if ((ISP_ABS_TBL_LSC & isptables_struct.update) ==
			ISP_ABS_TBL_LSC) { 
			if (copy_from_user(&ispccdc_lsc_tbl,
				(isptables_struct.lsc), sizeof(ispccdc_lsc_tbl))) {
				goto copy_from_user_err;
			}
			if (copy_from_user(&lsc_cf_t,
			(struct ispccdc_lsc_config *)(isptables_struct.lsc_cfg),
				sizeof(struct ispccdc_lsc_config))) {
				goto copy_from_user_err;
			}
			lsc_initialized=0;
		}
		if(sizeof(ispccdc_lsc_tbl)!= lsc_cf_t.size) {
			size_mismatch=1;
			ispccdc_load_lsc(lsc_cf_t.size);
			ispccdc_config_lsc(&lsc_cf_t);
			ispmmu_unmap(lsc_ispmmu_addr_temp);
			kfree(lsc_gain_table_temp);
			size_mismatch=0;					
		}
		else {
			size_mismatch=0;
 ...
 */
void
ispccdc_config_shadow_registers(void)
{
	return;
}
...
EXPORT_SYMBOL(ispccdc_config_crop);

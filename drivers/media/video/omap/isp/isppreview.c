...
/* Default spread value for Noise Filter*/
...
/*
 * Abstraction layer Preview configuration.
 */
int 
omap34xx_isp_preview_config(void *userspace_add)
{
	...
	if (userspace_add != NULL) {
		...
		if (( ISP_ABS_PREV_DRKFRAME & preview_struct.flag) ==
			ISP_ABS_PREV_DRKFRAME){
			isppreview_enable_drkframe(1);
			params->features |= ~(PREV_DARK_FRAME_SUBTRACT);
			}
		else {
		...
		if ((ISP_ABS_PREV_SHADINGCOMP & preview_struct.update) ==
			ISP_ABS_PREV_SHADINGCOMP) 
			isppreview_config_drkf_shadcomp
			(preview_struct.shading_shift);
	
		if ((ISP_ABS_PREV_HRZ_MED  & preview_struct.update) ==
			ISP_ABS_PREV_HRZ_MED){
			if (copy_from_user(&prev_hmed_t,
				(struct ispprev_hmed *)(preview_struct.prev_hmed),
		...
		if (( ISP_ABS_PREV_CFA & preview_struct.update) ==
			...
			if (copy_from_user(&prev_cfa_t,
				(struct ispprev_cfa *)(preview_struct.prev_cfa),
		...
		if ((ISP_ABS_PREV_CHROMA_SUPP & preview_struct.update) ==
			...
			if (copy_from_user(&csup_t,
				(struct ispprev_csup *)(preview_struct.csup),
		...
		if ((ISP_ABS_PREV_WB & preview_struct.update) ==
			...
			if (copy_from_user(&prev_wbal_t,
				(struct ispprev_wbal *)(preview_struct.prev_wbal),
		...
		if ((ISP_ABS_PREV_BLKADJ & preview_struct.update) ==
			...
			(struct ispprev_blkadjl *)(preview_struct.prev_blkadj),
		...
		if ((ISP_ABS_PREV_RGB2RGB & preview_struct.update) ==
			...
			if (copy_from_user(&rgb2rgb_t,
				(struct ispprev_rgbtorgb *)(preview_struct.rgb2rgb),
		...
		if ((ISP_ABS_PREV_COLOR_CONV & preview_struct.update) ==
			...
			if (copy_from_user(&prev_csc_t,
				(struct ispprev_csc *)(preview_struct.prev_csc),
		...
		if ((ISP_ABS_PREV_YC_LIMIT & preview_struct.update) ==
			...
			if (copy_from_user(&yclimit_t,
				(struct ispprev_yclimit *)(preview_struct.yclimit),
		...
		if ((ISP_ABS_PREV_DEFECT_COR & preview_struct.update) ==
			...
			if (copy_from_user(&prev_dcor_t,
				(struct ispprev_dcor *)(preview_struct.prev_dcor),
...
/*
 * Abstraction layer Tables update.
 */
int
omap34xx_isp_tables_update(void *userspace_add)
{
	struct isptables_update isptables_struct;
	u32 table_tt[ISP_NF_TABLE_SIZE];
	...
	if(userspace_add !=NULL){
		...
		if ((ISP_ABS_TBL_NF & isptables_struct.update) ==
			ISP_ABS_TBL_NF) { 
			NF_update=1;
			NF_enable=isptables_struct.flag;
			if (copy_from_user(&prev_nf_t,
				(struct ispprev_nf *)(isptables_struct.prev_nf),
				sizeof(struct ispprev_nf))) {
				goto err_copy_from_user; 
			} 
			if (copy_from_user(table_tt,(prev_nf_t.table),
				sizeof(table_tt))) {
				goto err_copy_from_user; 
			}					
		}
			...
			if ((ISP_ABS_TBL_REDGAMMA & isptables_struct.update) ==
				...
				if ((ISP_ABS_TBL_GREENGAMMA & 
					isptables_struct.update) ==
	...
	err_copy_from_user:
				printk(KERN_ERR);
				DPRINTK_ISPPREV("ISP ERR \n Preview Tables") 
				DPRINTK_ISPPREV("Copy From User Error \n");
				return -EINVAL;
}
...
isppreview_config_shadow_registers()
{
	...
	if(GG_update)
	{
		...
		for( ctr=0;ctr<ISP_GAMMA_TABLE_SIZE;ctr++)	
			omap_writel(greengamma_table[ctr],ISPPRV_SET_TBL_DATA);

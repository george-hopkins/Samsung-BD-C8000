...
#define OV3640_XCLK_HZ		16000000
...
const static struct ov3640_reg ov3640_common[2][100] = {
	...
	{ 0x3011, 0x00}, { 0x304c, 0x83}, { 0x3018, 0x38}, 
...
ov3640_clkrc(enum image_size isize, unsigned long xclk, struct v4l2_fract *fper)
{
	 ...
	 * Pixel Clk = fclk/Bit8Div/SenDiv/CLKdiv/4
	 ...
	 * SenDiv = 0x3010[4]
	 * CLKdiv = 3011[5:0] + 1
	 * Number of clks per frames / Pixel Clk = Frame rate
	 * All registers are fixed except for 0x300E register	 
	 * so we change the value of this register in order to achieved
	 * different frame rates.
	 * 0x300E = 64 - frame rate * clks per frame / 2600000
	 * the number 2600000 comes from the value of registers fixed	 	 	 	 
	 */
...
static int
ov3640sensor_cropcap(struct v4l2_cropcap *cropcap, void *priv)
{
	struct ov3640_sensor *sensor = (struct ov3640_sensor *) priv;
...
static int
ov3640sensor_get_crop(struct  v4l2_crop *crop, void *priv)
{
	struct ov3640_sensor *sensor = (struct ov3640_sensor *) priv;
...
static int
ov3640sensor_set_crop(struct  v4l2_crop *crop, void *priv)
{
	struct ov3640_sensor *sensor = (struct ov3640_sensor *) priv;
	...
	struct i2c_client *client = &sensor->client;
	int hstart, vstart;
	int hsize, vsize, hsize_l, vsize_l, hsize_h, vsize_h;
	int hratio, vratio, zoomfactor;
	int err;
	...
	if (sensor->isize == QXGA) zoomfactor = 1;
	else  zoomfactor = 2;
	...
	err = ov3640_write_reg (client, 0x3360, hsize_l);
	err = ov3640_write_reg (client, 0x3361, vsize_l);
	err = ov3640_write_reg (client, 0x3020, (hstart>>8)&0xFF);
	err = ov3640_write_reg (client, 0x3021, hstart&0xFF);
	err = ov3640_write_reg (client, 0x3022, (vstart>>8)&0xFF);
	err = ov3640_write_reg (client, 0x3023, vstart&0xFF);
	err = ov3640_write_reg (client, 0x3024, ((hsize)>>8)&0xFF);
	err = ov3640_write_reg (client, 0x3025, hsize&0xFF);
	err = ov3640_write_reg (client, 0x3026, ((vsize)>>8)&0xFF);
	err = ov3640_write_reg (client, 0x3027, vsize&0xFF);
...
static struct camera_sensor camera_sensor_if = {
	...
	cropcap:	ov3640sensor_cropcap,
	get_crop:	ov3640sensor_get_crop,
	set_crop:	ov3640sensor_set_crop,
};

...
omap34xxcam_set_isp_callback(struct omap34xxcam_device *cam, struct sgdma_state *sgdma)
{
	...
	isp_set_callback(CBK_CCDC_VD1, sgdma->callback, cam, sgdma->arg);
	...
	enum v4l2_field field)
{
	...
	if (!err) {
		...
		if (vb->baddr) {
			...
			dmac_flush_range(dma->vmalloc, (dma->vmalloc + (dma->nr_pages << PAGE_SHIFT)));
			outer_flush_range(__pa(dma->vmalloc), __pa(dma->vmalloc + (dma->nr_pages << PAGE_SHIFT)));
		}
	} else
		omap34xxcam_vbq_release(q, vb);
	...
	case VIDIOC_S_CTRL:
	  {
		...
		if(cam->cam_sensor->sensor_type == SENSOR_RAW) {
			...
			if(vc->id == V4L2_CID_BRIGHTNESS) {
				...
				omap34xx_isp_ccdc_config((void *)(vc->value));
				return 0;
			} else if(vc->id == V4L2_CID_PRIVATE_BASE+2) {
					omap34xx_isp_preview_config((void *)(vc->value));
					return 0;
			} else if(vc->id == V4L2_CID_PRIVATE_BASE+3) {
					omap34xx_isp_tables_update((void *) (vc->value));
					omap34xx_isp_lsc_update((void *) (vc->value));
					return 0;
			} else
#endif
				return -EINVAL;

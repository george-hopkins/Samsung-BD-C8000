...
/*
 * This macro enables the logic to detect battery presence
 * based on switch S10 #1 on 3430 SDP.
 * Physically inserting/removing the battery does not seem to
 * generate any interrupt.
 */
#undef BATT_PRES_DETECTION
#ifdef BATT_PRES_DETECTION
static int battery_present;
spinlock_t bci_spinlock;
...
#ifdef BATT_PRES_DETECTION
	spin_lock(&bci_spinlock);
	if (battery_present == 0) {
		printk(KERN_INFO "Error reading voltage from the battery\n");
		return -ENODEV;
	}
	spin_unlock(&bci_spinlock);
#endif
	...
	char *buf)
{
	...
	spin_lock(&bci_spinlock);
	if (battery_present == 0) {
		printk(KERN_INFO "Error reading current from the battery\n");
		return -ENODEV;
	}
	spin_unlock(&bci_spinlock);
#endif
	...
	char *buf)
{
	...
	spin_lock(&bci_spinlock);
	if (battery_present == 0) {
		printk(KERN_INFO "Error reading temperature"
			"from the battery\n");
		return -ENODEV;
	}
	spin_unlock(&bci_spinlock);
 ...
 * specifically USB_PRES (USB charger presence) CHG_PRES (AC charger presence)
 * events
 */
...
#ifdef BATT_PRES_DETECTION
		spin_lock_irqsave(&bci_spinlock, irqflags);
		battery_present = 1;
		spin_unlock_irqrestore(&bci_spinlock, irqflags);
#endif
		/* calling the battery presence event service function */
		if (twl4030battery_event.battery_presence)
		...
		while (!(value & PHY_DPLL_CLK) &&
			time_before(jiffies, timeout)) {
				...
				ret = twl4030_i2c_read_u8(TWL4030_MODULE_USB,
					&value, REG_PHY_CLK_CTRL_STS);
				if (ret)
					return ret;
		}
		twl4030_i2c_read_u8(TWL4030_MODULE_USB, &value,
			REG_PHY_CLK_CTRL_STS);
		if (!(value & PHY_DPLL_CLK))
			printk(KERN_ERR "Timeout setting T2 HSUSB PHY"
				" DPLL clock\n");
...
int twl4030battery_current(void)
{
	...
	spin_unlock(&bci_spinlock);
#endif
	curr = read_bci_val(T2_BATTERY_CUR);
	ret = twl4030_i2c_read_u8(TWL4030_MODULE_MAIN_CHARGE, &val,
		REG_BCICTL1);
	if (ret)
		return ret;
 ...
 * helper function
 *
 * Setup the twl4030 BCI and MADC module to measure battery
 * temperature
 *
 * @retval
 * 	0	On success
...
static int __init omap_battery_init(void)
{
	...
	ret = platform_driver_register(&omap_bci_battery_driver);
	if (ret) {
		printk(KERN_ERR  "BATTERY DRIVER:"

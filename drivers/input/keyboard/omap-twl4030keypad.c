...
static void twl4030_kp_scan(int release_all)
{
	...
	if (release_all)
		...
		ret =
			twl4030_i2c_read(TWL4030_MODULE_KEYPAD, new_state,
					code_reg, NUM_ROWS);
			if (ret < 0) {
				dev_warn(dbg_dev, "Could not read TWL4030 "
						"register %X - ret %d[%x]\n",
						code_reg, ret, ret);
				return;
			}
	...
	if (omap_kp_spurious(new_state[row]))
		...
		for (col = 0; col < NUM_COLS; col++) {
			...
			if (key < 0) {
#ifdef CONFIG_MACH_OMAP_3430LABRADOR
				/* Labrador has a TWL4030 GPIO (KBR5/KBC4)
				 * that is set to a persistent state and
				 * should be ignored.
				 */
				if (row == 5 && col == 4)
					continue;
...
#endif /*CONFIG_MACH_OMAP_3430LABRADOR*/

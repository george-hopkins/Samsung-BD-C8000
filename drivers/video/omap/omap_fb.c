...
#ifdef DEBUG
		printk(KERN_INFO "Switching FB to GFX pipeline \n");
#endif
		omap24xxfb_set_par(&saved_oinfo->info);
	} else {
#ifdef DEBUG
		printk(KERN_INFO "Switching FB to video pipeline%d \n", layer);
		printk(KERN_INFO "var.xres=%d, var.yres=%d \n",
			oinfo->info.var.xres, oinfo->info.var.yres);
		printk(KERN_INFO "var.xoffset=%d, var.yoffset=%d \n",
			oinfo->info.var.xoffset, oinfo->info.var.yoffset);
#endif
		mdelay(1);

		rotate = (oinfo->rotation_support) ? var->rotate : -1;
		omap2_disp_config_vlayer(layer, &pix, &crop, &win, rotate,
			omap24xxfb_mirroring);
		mdelay(1);
		omap2_disp_start_vlayer(layer, &pix, &crop, &win,
			(rotate >= 0) ? SMS_ROT_VIRT_BASE(0 , 0) :
				oinfo->info.fix.smem_start,
			rotate, omap24xxfb_mirroring);
		wait_for_reg_sync(SCHEDULE_WAIT, oinfo->timeout);
	}
...
static int
omap24xxfb_open(struct fb_info *info, int user)
{

...
/* Brodcom WLAN device, */
#define	BRODCOM_WLAN_VID		0x0a5c
#define	BRODCOM_WLAN_PID		0xbd13

static int IsBrodcomWLAN(struct usb_device *dev)
{
	int ret = 0;

	if(dev->descriptor.idVendor || dev->descriptor.idProduct )
	{
		if((dev->descriptor.idVendor == BRODCOM_WLAN_VID) && (dev->descriptor.idProduct == BRODCOM_WLAN_PID))
		{
			ret = 1;
		}		
	}
...
static int usb_classdev_add(struct usb_device *dev)
{
	...
	if(IsBrodcomWLAN(dev) == 1)
	{
		printk("\t Brodcom WLAN device detected\n");
	}
	else if(ReadUsbClass(dev) == 1)
	{
		printk("\tnot supported device connected \tManufacturer: %s \t Product : %s\n", dev->manufacturer,dev->product);
		kobject_uevent(&dev->usb_classdev->kobj, KOBJ_NOTSUPPORTED_DEV_ADD);
	}
	return 0;
}

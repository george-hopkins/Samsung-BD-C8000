		  ...
		  size_t len, int token, int maxpacket)
{
	...
	else if ( !usb_pipein(qtd->urb->pipe) &&
			(len != count) && ((u32)addr & 63) ) {
		/* This condition may not be happened.
		 * Because workaround code in usb hcd layer already replaced
		 * unaligned buffer before. */
		printk (KERN_CRIT "[CRITICAL] ehci-sdp: address misaligned,"
				"split this urb into two(or more) qtds."
				"(%08x %08x)\n", (u32)addr, len);
	}
#endif
	else {

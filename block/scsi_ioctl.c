...
int blk_verify_command(unsigned char *cmd, int has_write_perm)
{
	static unsigned char cmd_type[256] = {
        ...
        safe_for_read(GPCMD_V0_ENABLE_DIAG_MODE),
        safe_for_read(GPCMD_V0_GET_CHECKSUM),
        safe_for_read(GPCMD_V1_REPORT_KEY),
        safe_for_read(GPCMD_V1_READ_DVD_STRUCTURE),
        safe_for_read(GPCMD_V2_READ_RECORDER),
#endif
...
#if defined (CONFIG_MIPS_BCM_NDVD)
        safe_for_write(GPCMD_V0_FLASH_COMMAND),
        safe_for_write(GPCMD_V1_SEND_KEY),
        safe_for_write(GPCMD_V2_SEND_RECORDER),
#endif
	};
...
static int cdb_xfer_len(unsigned char *cdb, unsigned int sector_size)
{
    ...
    case GPCMD_SET_STREAMING:
        ...
        break;
    case 0xF3:  // by gyu : add by dracolee   :: for Loader Update
        xfer_len = 0x00;
        break;

    case 0xFF:  // by gyu : add by dracolee   :: for Testmode(Loader Adjust)
                xfer_len = 0x00;
                break;
    case 0xFE:  // by gyu : add by dracolee   :: for Saving Log
                xfer_len =  (cdb[10] << 8) | cdb[11];
//              xfer_len *= sector_size;             
                break;
		   ...
		   struct gendisk *bd_disk, unsigned int cmd, void __user *arg)
{
	...
	switch (cmd) {
		...
		case CDROM_SEND_PACKET: {
			...
			// modified by wskim 2009.11.22
			//printk(KERN_ERR"(original cgc.timeout is changed %d to %d  / cmd (%x) )\n", cgc.timeout, 15000, cgc.cmd[0]);
			if(cgc.timeout < 15000){
				cgc.timeout = 15000;
			}
			// end

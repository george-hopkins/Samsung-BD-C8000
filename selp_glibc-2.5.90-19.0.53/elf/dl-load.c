...
const char* map_lock (int locked_load_mode,
		      ElfW(Addr) mapstart, size_t len, size_t lock_len,
                      ...
                      struct numapolicy_info * policy)
{
  ...
  if (! (locked_load_mode & (RTLD_LOCK_DEPENDENT_LIB_PAGES
  ...
  else if (locked_load_mode & RTLD_LOCK_MLOCK)
    {
      ...
      if (mapat == MAP_FAILED)
	return N_("failed to map segment from shared object");
      if (mlock((caddr_t) mapat, lock_len) != 0)
	{
...
cannot allocate TLS data structures for initial thread");
	...
	/* Remember which part of the address space this object uses.  */
 	errstring = map_lock (locked_load_mode, mappref, maplength,
			      c->dataend - c->mapstart,
			      c->prot, 0, fd, c->mapoff, &(l->l_map_start),
			      c->policy);
                ...
                int locked_load_mode)
{
 ...
 no_file:
  ...
  return _dl_map_object_from_fd (name, fd, &fb, realname, loader, type, mode,
				 &stack_end, nsid, locked_load_mode);
}

...
print_cache (const char *cache_name)
{
  ...
  if (memcmp (cache->magic, CACHEMAGIC, sizeof CACHEMAGIC - 1))
    ...
    {
      unsigned int nlibs = READ_FIELD (cache->nlibs);
      size_t offset = ALIGN_CACHE (sizeof (struct cache_file)
				   + (nlibs * sizeof (struct file_entry)));
      /* This is where the strings start.  */
      cache_data = (const char *) &cache->libs[nlibs];
  ...
  if (format == 0)
    {
      ...
      /* Print everything.  */
      for (i = 0; i < nlibs; i++)
	print_entry (cache_data + READ_FIELD (cache->libs[i].key),
		     READ_FIELD (cache->libs[i].flags), 0, 0,
		     cache_data + READ_FIELD (cache->libs[i].value));
    }
  else if (format == 1)
    {
      uint32_t nlibs = READ_FIELD (cache_new->nlibs);
      printf (_("%d libs found in cache `%s'\n"), nlibs, cache_name);

      /* Print everything.  */
      for (i = 0; i < nlibs; i++)
	print_entry (cache_data + READ_FIELD (cache_new->libs[i].key),
		     READ_FIELD (cache_new->libs[i].flags),
		     READ_FIELD (cache_new->libs[i].osversion),
		     READ_FIELD (cache_new->libs[i].hwcap),
		     cache_data + READ_FIELD (cache_new->libs[i].value));
    }
  /* Cleanup.  */
...
save_cache (const char *cache_name)
{
  ...
  if (opt_format != 2)
	...
	+ cache_entry_old_count * sizeof (struct file_entry);
      ...
      WRITE_FIELD (file_entries->nlibs, cache_entry_old_count);
    }
  ...
  if (opt_format != 0)
	...
	(struct cache_file_new *) xmalloc (file_entries_new_size);
      ...
      WRITE_FIELD (file_entries_new->nlibs, cache_entry_count);
      WRITE_FIELD (file_entries_new->len_strings, total_strlen);
    }
  ...
  for (idx_old = 0, idx_new = 0, entry = entries; entry != NULL;
	...
	{
	  WRITE_FIELD (file_entries->libs[idx_old].flags, entry->flags);
	  /* XXX: Actually we can optimize here and remove duplicates.  */
	  WRITE_FIELD (file_entries->libs[idx_old].key, str_offset + pad);
	}
      if (opt_format != 0)
	{
	  /* We could subtract file_entries_new_size from str_offset -
	     ...
	     struct.  */
	  WRITE_FIELD (file_entries_new->libs[idx_new].flags, entry->flags);
	  WRITE_FIELD (file_entries_new->libs[idx_new].osversion,
		       entry->osversion);
	  WRITE_FIELD (file_entries_new->libs[idx_new].hwcap, entry->hwcap);
	  WRITE_FIELD (file_entries_new->libs[idx_new].key, str_offset);
	}
      ...
      /* Then the path.  */
      if (opt_format != 2 && entry->hwcap == 0)
	WRITE_FIELD (file_entries->libs[idx_old].value, str_offset + pad);
      if (opt_format != 0)
	WRITE_FIELD (file_entries_new->libs[idx_new].value, str_offset);

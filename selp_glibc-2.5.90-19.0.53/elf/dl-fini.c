...
/* Run after destructors.  */
void weak_function (*__dl_fini_trace) (const ElfW(Addr) addr, const char *name,
				       const ElfW(Phdr) *phdr,
				       ElfW(Half) phnum) = NULL;
      ...
      for (i = 0; i < nmaps; ++i)
	{
	  ...
	  if (l->l_init_called)
		  ...
		  || l->l_info[DT_FINI] != NULL)
		{
	      ...
	      /* Call the debugging hook after running all the code from the
	         object being unloaded.  */
	      if (__dl_fini_trace != NULL)
		(*__dl_fini_trace) (l->l_addr, l->l_name,
				    l->l_phdr, l->l_phnum);

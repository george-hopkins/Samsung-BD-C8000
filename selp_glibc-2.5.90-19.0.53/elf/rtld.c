...
static void
lock_exec_pages(ElfW(Addr) start, size_t memsz, int flags,
		const ElfW(Phdr) *ph)
{
  void *mapat, *newmap;
  int prot;
  ...
  if ((flags & RTLD_LOCK_MASK) == RTLD_LOCK_MLOCK)
    {
      ...
      if (mlock((caddr_t) start, memsz) != 0)
	{
  ...
  else
    {
      ...
      mapat = __mmap(0, memsz,
		     PROT_READ | PROT_WRITE, MAP_ANON | MAP_PRIVATE,
		     ANONFD, 0);
      if (mapat == MAP_FAILED)
	{
	  _dl_fatal_printf("Could not allocate temp memory for executable\n");
	  _exit(EXIT_FAILURE);
	}

      memcpy(mapat, (void *) start, memsz);

      if (munmap((void *) start, memsz) != 0)
	{
	  _dl_fatal_printf("Could not unmap the old executable\n");
	  _exit(EXIT_FAILURE);
	}

      newmap = __mmap((caddr_t) start, memsz,
		      PROT_EXEC | PROT_READ | PROT_WRITE,
		      MAP_FIXED | MAP_ANON | MAP_PRIVATE,
		      ANONFD, 0);
      if (newmap == MAP_FAILED)
	{
	  _dl_fatal_printf("Could not allocate memory for the executable\n");
	  _exit(EXIT_FAILURE);
	}

      memcpy((void *) start, mapat, memsz);
      munmap(mapat, memsz);

#ifdef CODE_AREA_CHANGED
      /* Handle cache coherency issues. */
      if (ph->p_flags & PF_X)
	CODE_AREA_CHANGED(start, memsz);
#endif
	 ...
	 ElfW(Addr) *user_entry)
{
  ...
  if (*user_entry == (ElfW(Addr)) ENTRY_POINT)
	...
	else if (! strcmp (INTUSE(_dl_argv)[1], "--mlock"))
	  {
	    GLRO(dl_locked_load_mode) = (RTLD_LOCK_LIB_PAGES | RTLD_LOCK_MLOCK);

	    ++_dl_skip_args;
	    --_dl_argc;
	    ++INTUSE(_dl_argv);
	  }
	else if (! strcmp (INTUSE(_dl_argv)[1], "--readload"))
	  {
	    GLRO(dl_locked_load_mode) = (RTLD_LOCK_LIB_PAGES | RTLD_LOCK_READ);
...
\n\
  ...
  --inhibit-rpath LIST  ignore RUNPATH and RPATH information in object names\n\
      ...
      else
	{
	  ...
	  _dl_map_object (NULL, rtld_progname, 0, lt_library, 0,
			  __RTLD_OPENEXEC, LM_ID_BASE, GLRO(dl_locked_load_mode));
	  HP_TIMING_NOW (stop);
  ...
  for (ph = phdr; ph < &phdr[phnum]; ++ph)
      ...
      case PT_LOAD:
	{
	  ...
	  if (GLRO(dl_locked_load_mode))
	    lock_exec_pages (mapstart, allocend - mapstart,
			     GLRO(dl_locked_load_mode), ph);
	}
	break;
...
#define STRING(s) __STRING (s)
  ...
  HP_TIMING_NOW (start);
  _dl_map_object_deps (main_map, preloads, npreloads, mode == trace, 0, GLRO(dl_locked_load_mode));
  HP_TIMING_NOW (stop);
  HP_TIMING_DIFF (diff, start, stop);
  HP_TIMING_ACCUM_NT (load_time, diff);
...
#define VERNEEDTAG (DT_NUM + DT_THISPROCNUM + DT_VERSIONTAGIDX (DT_VERNEED))
  ...
  if (main_map->l_info[ADDRIDX (DT_GNU_LIBLIST)]
	...
	prelinked = true;
      ...
      if (__builtin_expect (GLRO(dl_debug_mask) & DL_DEBUG_LIBS, 0))
	  {
		  ...
		  {
			  if (r_list != r_listend)
				  _dl_printf ("Last loaded library is %s, checksum 0x%0*Zx, timestamp 0x%0*Zx, addr 0x%0*Zx\n",
						  (*r_list)->l_name,
						  sizeof (ElfW(Word)) * 2,
						  (*r_list)->l_info [VALIDX(DT_CHECKSUM)]
						  ? (*r_list)->l_info [VALIDX(DT_CHECKSUM)]->d_un.d_val : 0,
						  sizeof (ElfW(Word)) * 2,
						  (*r_list)->l_info [VALIDX(DT_GNU_PRELINKED)]
						  ? (*r_list)->l_info [VALIDX(DT_GNU_PRELINKED)]->d_un.d_val : 0,
						  sizeof (ElfW(Word)) * 2, (*r_list)->l_addr);
			  else
				  _dl_printf ("No more libraries loaded.\n");
			  if (liblist != liblistend)
				  _dl_printf ("Last expected library is %s, checksum 0x%0*Zx, timestamp 0x%0*Zx\n",
				  ...
				  _dl_printf ("No more libraries expected.\n");
		  }
	  }
    }

  r = (struct r_debug *) NULL;
  if (! main_map->l_info[DT_NEEDED])
  ...
  else
    {
      ...
      r = _dl_debug_initialize (GL(dl_rtld_map).l_addr, LM_ID_BASE);
      l = main_map;
...
process_envvars (enum mode *modep)
{
  ...
  while ((envline = _dl_next_ld_env_entry (&runp)) != NULL)
	...
	case 11:
	  ...
	  if (memcmp (envline, "LOCK_MEMORY", 11) == 0)
	    {
	      if (strcmp (&envline[12], "MLOCK") == 0)
		GLRO(dl_locked_load_mode) = RTLD_LOCK_LIB_PAGES | RTLD_LOCK_MLOCK;
	      else
		GLRO(dl_locked_load_mode) = RTLD_LOCK_LIB_PAGES | RTLD_LOCK_READ;
	    }

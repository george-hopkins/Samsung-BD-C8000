  ...
  if (av != &main_arena) {
	...
	max_total_mem = mmapped_mem + arena_mem + sbrked_mem;
	 ...
	 up, too, although the chunk is marked in use. */
      ...
      if (old_size >= MINSIZE) {
	int fence_size = (old_size & MALLOC_ALIGN_MASK) + 2*SIZE_SZ;
	old_size &= ~MALLOC_ALIGN_MASK;
	set_head(chunk_at_offset(old_top, old_size), fence_size|PREV_INUSE);
	set_foot(chunk_at_offset(old_top, old_size), fence_size);

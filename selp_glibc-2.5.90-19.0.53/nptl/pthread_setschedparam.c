     ...
     const struct sched_param *param;
{
	...
	if (policy == SCHED_OTHER){
		...
		if ( param->sched_priority < -20 && param->sched_priority > 19 ){
			printf ("SELP:ERROR! Range of priority that you can input is between -20 and 19 \r\n");
			return errno;
		}
		...
		if (setpriority(which, pd->tid, param->sched_priority) ){
			printf ("SELP:setpriority() is ERROR. It seems that System not support setpriority()\n");
			result = errno;
		}
		return result ;
	}
...
if (policy == SCHED_FIFO || policy == SCHED_RR){
  ...
  /* low level locking of structure variable called "pd" */
  lll_lock (pd->lock);
